...
const REPO_COLUMNS = [
	"id", "organization_id", "provider", "owner", "name", "external_id", "url",
	"description", "default_branch", "is_private", "config", "is_active",
	"org_privilege", "last_synced_at", "created_at", "updated_at",
] as const;
const ISSUE_COLUMNS = [
	"id", "repo_id", "number", "external_id", "title", "body", "state",
	"author_login", "author_avatar_url", "assignee_logins", "labels",
	"comment_count", "url", "external_created_at", "external_updated_at",
	"closed_at", "created_at", "updated_at",
] as const;
const PULL_COLUMNS = [
	"id", "repo_id", "number", "external_id", "title", "body", "state",
	"is_draft", "author_login", "author_avatar_url", "head_branch", "base_branch",
	"labels", "comment_count", "additions", "deletions", "changed_files", "url",
	"external_created_at", "external_updated_at", "merged_at", "closed_at",
	"created_at", "updated_at",
] as const;
const INSTALLATION_COLUMNS = [
	"id", "organization_id", "installation_id", "account_id", "account_login",
	"account_type", "account_avatar_url", "repository_selection", "permissions",
	"created_at", "updated_at",
] as const;
const GRANT_COLUMNS = [
	"id", "user_id", "provider_id", "github_user_id", "github_login",
	"access_token", "refresh_token", "access_token_expires_at",
	"refresh_token_expires_at", "scope", "created_at", "updated_at",
] as const;
const INTEGRATION_COLUMNS = [
	"id", "board_id", "type", "config", "is_active", "created_at", "updated_at",
] as const;
...
export const importRepositoryDumpEffect = Effect.fn("Domain.importRepositoryDump")(
	function* (sql: Sql, actor: string) {
		return yield* Effect.tryPromise({
			try: () =>
				sql.begin(async (tx) => {
					// --- preflight reads (source is never written) ---
					const repos = (await tx`SELECT * FROM kaneo_src.repo ORDER BY id`) as unknown as DumpRow[];
					const issues = (await tx`SELECT * FROM kaneo_src.repo_issue ORDER BY id`) as unknown as DumpRow[];
					const pulls = (await tx`SELECT * FROM kaneo_src.repo_pull_request ORDER BY id`) as unknown as DumpRow[];
					const installations = (await tx`SELECT * FROM kaneo_src.organization_github_installation ORDER BY id`) as unknown as DumpRow[];
					const grants = (await tx`SELECT * FROM kaneo_src.github_user_grant ORDER BY id`) as unknown as DumpRow[];
					const integrations = (await tx`SELECT * FROM kaneo_src.integration ORDER BY id`) as unknown as DumpRow[];
					...
					for (const pull of pulls) {
						...
						if (pullNumbers.has(key)) throw new Error("Duplicate pull request number");
						pullNumbers.add(key);
					}
					...
					for (const installation of installations) {
						...
						if (
							...
							planned.push({
								...
								payload: { id: installation.id, row: installation, origin: "import" },
							});
					}
					// Board→org resolution comes from the dump itself; integrations
					// are board-owned so their events land in the owning org.
					const boards = (await tx`SELECT id, organization_id FROM kaneo_src.board`) as unknown as Array<{
						id: string;
					...
					for (const integration of integrations) {
						...
						if (
							org &&
							prior.integration.get(String(integration.id)) !== rowDigest(integration)
						) {
					...
					for (const [org, events] of byOrg) {
						const [counter] = await tx`UPDATE org_event_counter SET seq=seq+${events.length}
