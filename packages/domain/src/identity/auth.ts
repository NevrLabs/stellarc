...
/** Authenticate a raw API key (READ-ONLY — rework c10 defects 4/5).
 * The c9 implementation minted principal + identity_grant rows from the key
 * ceiling on every authentication, which made the capabilities.ts
 * intersection vacuous (structural ⊇ ceiling by construction) and violated
 * D12 (domain writes with no events on the auth path). Grants now exist only
 * via createApiKey/importer mutations with their lifecycle events; this
 * function reads state and advances only the operational rate counter
 * (§2-exempt auth storage). The indexed equality on the stored digest is the
 * comparison (constant-time by construction over the b-tree lookup);
 * disabled/expired/banned keys are denied before any scope derivation
 * (T04/T05). The conditional UPDATE keeps concurrent authentications from
 * exceeding the rate limit: a key at its limit updates zero rows and the
 * caller is denied below. Returns null on any failure. */
export async function authenticateApiKey(
	...
	const refillCap =
		key.rate_limit_max !== null && key.rate_limit_max !== undefined
			? key.rate_limit_max
			: null;
	// Atomic rate gate (T05): row lock serializes concurrent callers; the
	// window/remaining semantics live entirely in SQL. Operational counter
	// write only — no principal, no identity_grant, no domain events.
