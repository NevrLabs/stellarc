...
export async function importWork(
	...
	for (const board of data.boards) {
		...
		if (board.last_task_number < 0) {
			report.errors.push(
				"preflight: negative last_task_number in boards table",
			);
		}
	...
	for (const board of data.boards) {
		const [clash] = (await sql`
			...
			WHERE organization_id = ${board.organization_id}
				AND lower(slug) = ${board.slug.toLowerCase()} AND id <> ${board.id}`) as unknown as Array<{
			id: string;
		}>;
		if (clash) {
			report.errors.push(
	...
	await sql
		.begin(async (tx) => {
			for (const entry of preflight) {
				...
				WHERE source_id = ${sourceId} AND table_name = 'board' AND source_pk = ${board.id}`) as
					| Array<{ digest: string }>
					| unknown as Array<{ digest: string }>;
				if (existing && existing.digest === entry.digest) continue;

				const [orgExists] = (await tx`
				SELECT id FROM organization WHERE id = ${board.organization_id}`) as unknown as Array<{
					id: string;
				}>;
				if (!orgExists) {
					report.errors.push(
				...
				for (
					...
					const definition = STATUS_DEFINITIONS.find(
