...
- Authorization, budgets, retention, node ownership, context generation, and concurrent mutation semantics are not yet defined strongly enough to keep Axis canonical.
...
- Current lineage is not a typed graph:
  - `SessionForked` stores a string `fork_type` and a single parent/child pair: `crates/axis/src/event.rs:115-137`.
  - `SessionView` collapses forks and handovers into one `parent_session_id`: `crates/axis/src/views/session.rs:66-74,236-258,290-300`.
  - SQLite likewise stores only `parent_session_id`: `crates/axis/src/log.rs:744-766`.
- Current subsession creation is not an acceptable atomic lineage write:
  ...
  See `crates/axis/src/server/routes/sessions.rs:2048-2125`.
- The current route has no depth check at all: `crates/axis/src/server/routes/sessions.rs:1993-2046`.
...
5. A prohibition on activity providers starting workflow runs outside Axis’s typed `workflow.run` operation.
...
Creation must durably commit, in one Axis transaction:
...
No node or edge may become visible independently. Orbit must not materialize the workspace or spawn an agent before that transaction commits.
...
Axis needs one durable **causal execution budget account** shared by the root and all descendants, regardless of edge type. It must cover at least:
...
The causal subgraph must be a DAG. Axis must reject:
...
A durable graph edge must never contain a host path as identity. Paths are Orbit-local projections of logical resource IDs.
...
- translation of logical repo/vault/artifact attachments through Orbit’s resolver;
- session move/recovery as a new fenced materialization generation;
- rejection of writes from stale Orbit/runtime attempts.

Flat layout is acceptable only if no correctness path scans the directory. Axis and the Orbit reconciliation index must enumerate sessions; filesystem enumeration is repair-only.
...
- node-local cleanup is driven by a Axis tombstone plus fenced Orbit reconciliation and acknowledgment;
...
“Axis is the single writer” does not eliminate races between async requests, scheduler actions, cancellation, retention, and Orbit reconciliation.
...
Each session workspace needs one Orbit reconciliation owner per materialization generation. `lineage.json` and `CONTEXT.md` must be written by temporary file plus flush/fsync as supported, atomic rename, and parent-directory durability where required. Readers must see either the prior complete generation or the new complete generation, never a partial file.
...
If Orbit creates convenience links, their namespace can collide with user files. Define a reserved Stellarc-managed directory such as `.stellarc/links/`, create links only when supported and policy permits, and never overwrite user content.
...
> Every session is an independent durable resource identified by an opaque, globally unique `session_id`. On a node where it is materialized, Orbit places its workspace at:
>
> ```text
...
>
> Session paths are Orbit-local projections, not durable identifiers or API contracts. Axis stores logical resource IDs and placement attempts, never authoritative host paths. Orbit validates `session_id` as one path-safe component, resolves it beneath the configured organization root, and rejects traversal, absolute paths, separator-bearing IDs, containment escapes, and stale materialization attempts.
>
> Correctness paths enumerate sessions from Axis projections or Orbit’s reconciliation index. They MUST NOT depend on scanning `sessions/`. The flat physical layout is therefore replaceable by a future internal sharding scheme without changing session identity or graph semantics.

## Canonical lineage graph

> Axis’s durable event log and projections are the sole authority for lineage. Lineage is a directed typed multigraph whose execution nodes include `Session` and `WorkflowRun`.
>
> V1 edge types are:
>
...
>
> Every execution node has zero or one immutable causal parent. Causal edges form a DAG. Axis rejects self-edges, a second causal parent, cycle-forming edges, missing or tombstoned endpoints, incompatible endpoint types, and cross-organization edges. Noncausal edges do not affect authority, depth, budgets, cancellation, or retention inheritance.
>
> Edge creation and node creation are one atomic Axis operation. The transaction records node identity, organization/project ownership, typed causal edge, capability envelope, depth values, budget reservation, workspace materialization intent, and operation idempotency key before Orbit receives any materialization or spawn command.
...
>
> Axis rejects any agent spawn whose resulting depth exceeds `3`. Thus the maximum chain is main=`0`, sub1=`1`, sub2=`2`, sub3=`3`.
>
...
>
> Depth is checked and recorded by Axis in the same transaction that creates the child. Orbit, clients, generated files, and caller-supplied depth values are not authoritative.
...
>
> Workflow definitions remain nonrecursive as required by ADR 0013. Activities and providers MUST NOT start workflow runs except through Axis’s typed and authorized `workflow.run` operation.
>
> Axis additionally tracks `workflow_nesting_depth` over the full causal ancestry, including workflow runs separated by workflow-created agents. A root invocation has depth `0`; starting another workflow from any descendant of that active run increments it by `1`.
>
...
> Axis authorizes every causal transition using durable canonical endpoint ownership and the current authenticated principal.
>
...
>
> Axis rechecks ownership, revocation, exact typed resource scope, and runtime attempt at workflow start, each step dispatch, agent spawn, resume, completion delivery, context traversal, and generated-context refresh. Missing capability envelopes fail closed on agent-facing operations.
>
...
>
> Axis atomically reserves budget before committing a spawn or dispatch intent. Insufficient budget rejects the operation without creating a node or side effect. Recovery and retry reuse the existing reservation and attempt identity. Cancellation releases only unused reservations; consumed resources remain charged. A child may receive a narrower sub-budget but cannot increase the root account or reset accounting through a workflow boundary.

## Derived lineage and context files

> Orbit may generate these convenience projections inside a materialized session workspace:
>
> ```text
> .stellarc/lineage.json
> .stellarc/CONTEXT.md
> ```
>
> They are caches derived from Axis’s canonical graph. They are never accepted as authority for identity, authorization, depth, budget, placement, retention, or recovery.
>
> Each generation records the schema version, Axis graph revision/event watermark, session ID, causal root and parent, typed ancestry, `agent_spawn_depth`, `workflow_nesting_depth`, logical resource references, and any truncation status.
>
...
>
> Orbit renders ancestors in deterministic root-to-parent order, deduplicates visited nodes, and uses a visited set even though the causal graph is required to be acyclic. Noncausal reference cycles are not traversed transitively.
>
> `CONTEXT.md` labels inherited text as untrusted context data, not system policy. Axis-side revocation or ownership changes invalidate affected snapshots. Orbit regenerates lazily at create/resume or before context use against the current graph revision.
>
> Orbit writes each file to a temporary sibling, flushes it, atomically renames it over the prior generation, and durably records the materialization generation as supported by the host filesystem. Readers observe either the prior complete snapshot or the new complete snapshot.
...
>
> Convenience links, when enabled, live only under `.stellarc/links/`. Orbit never overwrites user content, never follows a link for authorization or deletion, and resolves all actual access through logical resource bindings and sandbox bind mounts. Unsupported platforms omit the links without changing behavior.

## Placement, recovery, and concurrent updates

> Every workspace materialization and runtime has a Axis-issued attempt epoch. Exactly one Orbit owns reconciliation for `(session_id, materialization_epoch)`. A moved, recovered, cancelled, archived, or tombstoned session fences all prior epochs; stale Orbits and runtimes may not modify the workspace, publish generated context, emit completion, or perform effects.
>
> All Axis graph mutations carry an operation idempotency key and are serialized through the authoritative event-log transaction. Duplicate operations return the prior durable result. Competing mutations re-evaluate graph invariants and budget availability in the committing transaction.
>
> Workspace realization is reconciliation, not authority: Orbit may report `materialized`, `missing`, `stale`, `orphaned`, or `cleanup_pending`, but it may not invent graph nodes or edges from local disk contents.
...
>
> Workspace cleanup is initiated by a durable Axis tombstone and executed by the currently fenced Orbit materialization owner. Cleanup is acknowledged and retryable. It deletes only the named session workspace and never follows symlinks or recursively infers descendants from filesystem structure.
...
> - crash between node/edge/budget commit and Orbit materialization reconciles without orphan authority;
...
> - Axis restart reproduces identical graph projections and Orbit regenerates identical lineage artifacts;
