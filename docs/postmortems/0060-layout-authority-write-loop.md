# 0060 — Repeated equivalent Dockview events caused a Hall write loop
...
The writer treated every event as new work. Object identity and ordinary JSON insertion order are not reliable layout identity, so an authority/reload cycle could repeatedly feed equivalent snapshots back into Hall.
...
- Let the restore path adopt an authoritative Dockview snapshot without writing that programmatic restore back to Hall.
