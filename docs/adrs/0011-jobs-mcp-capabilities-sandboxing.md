...
Relates to: ADR 0002 (layer boundary), ADR 0008 (Axis/Orbit split, wire
protocol), ADR 0010 (Axis auth), ADR 0006 (setup adapter).

## Decisions

### 1. JobRunner is a role of stellarc-orbit, not a separate binary

`Hello` gains `NodeRole` — `AgentRuntime`, `JobRunner`, or both. Axis routes
work by role. Same binary, same iroh transport, same frame protocol.

- `AxisFrame` += `DispatchJob`, `CancelJob`.
- `OrbitFrame` += `JobOutput`, `JobResult`.
- Orbit gains a `JobTable` paralleling `RuntimeTable`. The idle-reaper pattern
  (shipped 2026-07-11, `3fd7d2f`) applies to both tables.

### 2. iroh is the overlay network

All inter-node Stellarc traffic rides iroh QUIC (e2e-encrypted, NAT-traversing,
keyed by node id). No WireGuard/Netbird/Tailscale for Stellarc traffic. A new
build node (e.g. LXC) needs only: orbit binary + toolchains + Axis's iroh node
...
> but eligible runtimes also receive the schema-aware `stellarc` CLI over the
> same Orbit-mediated operation gateway. CLI and MCP call one typed Axis
> operation seam and capability decision; neither wraps operator HTTP or holds
> the installation token.

Axis exposes an MCP server endpoint. The setup adapter injects it into each
session's `.mcp.json` (native path per ADR 0006 §9). Tools: `list_nodes`,
`run_job`, `get_job`; later `run_workflow`. The human-facing `stellarc` CLI is
...
- signed by Axis, validated on every call — fail closed.
...
returns `workflow_id` immediately; Axis pushes a `WorkflowComplete` event as a
...
1. Job dispatch — proto frames + orbit JobTable + Hello roles. Alone unblocks
   remote compilation offload.
2. MCP server on Axis.
3. Capability model.
...
Phase 1 touches `crates/proto/src/frames.rs` and orbit internals — the same
surfaces as in-flight cards ARCH-E (orbit ingestion, extends frames) and
