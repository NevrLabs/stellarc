...
- Relates to: ADR 0002 §6 session lineage, ADR 0008 Axis/Orbit ownership, ADR 0010 authorization, ADR 0017 remote execution

## 1. Context

ADR 0005 stores a main session at `~/.stellarc/<org>/sessions/<id>/` but nests each sub-session inside its parent. That makes filesystem ancestry carry control-plane meaning:
...
More importantly, directory nesting makes lineage node-local filesystem state even though Axis's event log is already authoritative for session relationships. It couples child lifecycle to parent retention, complicates relocation and cross-node execution, makes cleanup recursively destructive, and makes workflow-started agents difficult to represent without pretending they are subagents.

The implementation already resolves every session space directly as `<stellarc_home>/<organization_id>/sessions/<session_id>` in `BridgeManager::space_path` (`crates/axis/src/server/bridge_mgr.rs:97-102`). `SessionForked` and `SessionView.parent_session_id` already represent lineage in Axis (`crates/axis/src/event.rs:115-137`, `crates/axis/src/views/session.rs:236-258`). The accepted filesystem ADR contradicts both the correct authority boundary and the current path implementation. The current implementation does not yet enforce a spawn-depth limit and overloads `SessionForked { fork_type: "sub" }` for agent delegation.
...
```text
~/.stellarc/<org>/sessions/<session_id>/
```
...
Axis owns a typed invocation graph over sessions and workflow runs. Orbit materializes each assigned session independently and writes rebuildable context projections into its flat session space. The graph and event log are canonical; files and optional symlinks are never authority.
...
**Doctrine:** session storage is flat; agent delegation and workflow execution are distinct typed graphs; Axis enforces graph invariants before Orbit performs any host effect.
...
1. The portable locator is `(organization_id, session_id)`, not an absolute path. `node_id` identifies the Orbit that currently materializes the bytes.
2. Orbit derives the path from validated identifiers. User-controlled titles, prompts, workflow names, and agent names never enter path components.
3. A session ID is an opaque bounded identifier and must pass component-safety validation before any host effect.
4. Archiving or deleting one session never recursively archives or deletes related sessions. Axis emits explicit lifecycle operations for every affected session.
5. Relocation or rematerialization creates the same flat relative path on the destination node. Lineage needs no rewrite.
6. Symlinks are optional operator ergonomics only. They are absent in v1 unless a demonstrated need justifies them. If later enabled, they live under reserved `.stellarc/links/`, never overwrite user content, are relative, same-organization, cycle-safe, rebuildable, and ignored by authorization, retention, and recovery.
7. Correctness paths enumerate sessions through Axis or Orbit's reconciliation index, never by scanning `sessions/`. The flat physical shape is an internal implementation choice rather than a public identity contract.
...
Axis rejects an `agent_spawn` when the parent's projected depth is 3. Creation is one serialized operation that validates and durably records the child session, organization, capability envelope, relation, derived root/depth, and materialization intent before Orbit creates files or starts a process.
...
Depth is a Axis policy field derived from accepted typed events. Orbit metadata and `lineage.json` cannot grant permission or lower depth. Missing, corrupt, cross-organization, cyclic, or ambiguous ancestry fails closed for new agent spawning.

Imported sessions may contain deeper or incomplete historical ancestry. Stellarc preserves and displays that history but does not permit a session at derived depth 3 or greater to create another `agent_spawn`.
...
Axis's graph is canonical. Orbit generates two rebuildable projections in every materialized session space:

- `.stellarc/lineage.json`: versioned machine-readable graph snapshot;
- `.stellarc/CONTEXT.md`: concise agent/operator orientation generated from the same snapshot. Runtime adapters explicitly surface this managed file to agents; Stellarc does not claim a user-owned root filename.
...
- No graph invariant assumes relatives are colocated. Scheduling policy may prefer colocation, but context and artifacts cross nodes through authorized Axis/Orbit protocols, not filesystem-relative traversal.
- Every workspace materialization and runtime has a Axis-issued attempt epoch. Exactly one Orbit reconciles `(session_id, materialization_epoch)`; relocation, recovery, cancellation, archive, or tombstone fences stale Orbits and runtimes from writing files, publishing context, completing, or performing effects.
...
4. Detect any already-nested session spaces. For each child, stop its runtime, move or rematerialize it at the flat path, update Orbit's reconciliation locator, verify its jj workspace, and only then remove the old directory. Never follow symlinks during migration.
5. Generate `.stellarc/lineage.json` and `.stellarc/CONTEXT.md` from Axis and verify transitive descendants, organization filtering, atomic replacement, and stale-revision repair.
...
- Axis's durable graph is the sole lineage authority, consistent with the control-plane/Orbit boundary.
...
- Axis needs typed invocation events and stronger graph projections instead of one overloaded parent field.
- Orbit must reconcile context projections and nested-path migration.
- Agents cannot infer lineage from `pwd`; they use generated context or Axis APIs.
...
Rejected. Only Axis's separately authorized workflow-run operation can create workflow provenance. Workflow graphs remain finite and non-recursive, active workflow ancestry is cycle-checked, and all runs are budgeted.
