# ADR 0002: Stellarc is a multi-node agent fleet control plane
...
- Status: **PARTIALLY SUPERSEDED** — §3 (Identity/Context/Session model) and
  ...
  event log, orbit layer boundary, iroh/UDS transport, scheduler, §2, §6, §10,
...
> **Read this document in full before writing any Stellarc code.** It is the
...
> **Stellarc is a multi-node agent fleet control plane: a Rust-native single-
...
### 1.2 What Stellarc is NOT

- **Not an agent runtime.** Stellarc does not implement an agent loop, context
...
2. **Interactive agency.** Unlike Paperclip's heartbeat/ticket model (agents are
   black boxes that produce output on a schedule), every Stellarc agent session is
...
3. **Heterogeneous complementary runtimes.** Provider harnesses (Claude Code,
   ...
   OpenClaw) handle orchestration and judgment. Stellarc routes by task shape.
...
1. **Layer boundary:** orchestration/state → control plane; any host effect
   (process, file, PTY, port, install) → the host's orbit. Never blur it (§2.1).
2. **One durable process per host:** the orbit is the only durable, stateful
...
└───────────────────────────────┬──────────────────────────────────────────┘
        ...
        │                        │                        │
┌───────▼────────┐      ┌────────▼───────┐      ┌─────────▼──────┐
│ LAYER 2 ORBIT  │      │ LAYER 2 ORBIT  │      │ LAYER 2 ORBIT  │
│ (node agent,   │      │                │      │                │
│  one per host) │      │                │      │                │
│  - sole host-  │      │                │      │                │
│    level proc  │      │                │      │                │
...
Layer 2 (orbit) required.** This dividing line is absolute and is the reason
Layer 2 exists. The control plane is one logical authority coordinating many
hosts; host effects are inherently per-host and belong to that host's orbit. The
control plane process has no business spawning agents or touching a node's
filesystem even when co-located — it issues commands; the orbit performs them.
...
internals. All host execution flows through orbits via the command/event protocol
...
- **Local/co-located nodes → Unix domain socket** at
  `~/stellarc/system/control.sock` (mode 0600). The OS authenticates the peer
...
Stellarc adopts **position C** (hybrid local-first). Data splits into two planes
...
- **Session**: one conversation/work unit within a context. A **main session**
  ...
  (`~/stellarc/sessions/<session_id>/`, see §5); a **worker session** (the agent
...
- **Hard-enforced (by construction):**
  - Credentials: a session only receives credentials scoped to its context. The
    orbit injects only the context's secrets into a worker's environment.
  - Workspaces: a session can only read/write within its own
    `~/stellarc/sessions/<session_id>/` tree and its context's designated
...
Stellarc is multi-tenant and human-in-the-loop by design. The full scope
...
- **`assignedId` + `assignedKind: "agent" | "user"`** on cards/sessions/tasks:
  work may be assigned to an agent OR a human. Stellarc is agent-native but
...
- **Nodes/orbits** are granted to orgs by an org owner (`nodes.orgs`; extends the
...
data-model cut that lets Stellarc stay centralized for the MVP while keeping the
...
with a derived in-memory view (§2.4). "Owned by orbit" rows are host effects.
...
| Node desired-state (prereqs) | Control plane (`nodeDesiredState`) | §7; orbit reconciles |
| Host command queue + leases | Control plane (`hostCommands`) | §10, §10.5–10.6 |
| Skill/MCP library catalog | Control plane (`skills`,`mcpServers`) | §9; bytes materialized to disk by orbit |
...
| **Process spawn / supervision** | **Orbit** | Layer 2 |
| **PTY / terminal** | **Orbit** | `Bun.Terminal` or Rust PTY; §18 |
| **Filesystem authority + workdir lifecycle** | **Orbit** | path-guarded; §5 |
| **Skill/MCP/cred materialization to disk** | **Orbit** | library + scoped activation + creds; §5, §9 |
| **Knowledge-vault sync** | **Orbit** | jj pull/push on webhook + cron; §8 |
| **Desired-state reconciliation (installs)** | **Orbit** | gh/git/jj/gitnexus/…; §7 |
| **Port allocation** | **Orbit** | netns + forwarding; §12.4 |
| **Sandbox lifecycle** | **Orbit** | host-direct / bwrap / Docker; §12 |
| **Artifact byte serving + blob store** | **Orbit** | HTTP file server + content-addressed blobs; §17 |
| Agent loop / context mgmt / tools | The agent (Hermes etc.) | NOT Stellarc |

---

## 5. Filesystem hierarchy and workspace management — ⚠️ WITHDRAWN, see ADR 0005

> **Superseded by ADR 0005 (2026-07-01).** The flat `~/stellarc/sessions/`
> layout below is replaced by an org-scoped `~/.stellarc/<org>/sessions/`
...
Stellarc (via the orbit) owns every agent's working directory. On a new session the
orbit creates a scoped **session space** and runs the agent inside it, so the
...
| Term | Meaning |
|---|---|
| **Session space** | A session's whole directory: `~/stellarc/sessions/<session_id>/` — holds repos, artifacts, metadata, symlinks. NOT itself a repo. |
...
| **Origin repo** | `~/stellarc/repos/<org>/<repo>/` — the canonical jj repo all sessions fork (workspace-add) from. One shared history per repo. |
...
the session space's `.stellarc/artifacts/`, **never inside a `repos/<org>/<repo>/`
working copy**, so session output never pollutes a repo's `jj status`.

### 5.1 The `~/stellarc/` tree (exact; this layout is normative)

```text
~/stellarc/                               # Stellarc-managed root, one per node
...
│       ├── .stellarc/                    # session metadata — NOT a repo
...
├── skills/                              # Stellarc-managed skill LIBRARY (ALL managed skills)
│   └── <skill_id>/                      #   bytes materialized by orbit; READ-ALL (§9)
├── mcp/                                 # Stellarc-managed MCP LIBRARY (ALL managed servers)
│   └── <mcp_id>/                        #   READ-ALL for discovery (§9)
├── creds/                               # credential materialization
│   └── _context-<context_id>/           #   orbit WRITES scoped; agents READ their context only
...
- **A session space holds repos AND artifacts, kept separate.** `repos/<org>/<repo>/`
  is a jj workspace (versioned repo work); `.stellarc/artifacts/` is session output
...
- **Sub-sessions nest** as `sessions/<main>/<sub_id>/` with the same session-space
  shape (own `.stellarc/`, own `repos/`). The whole session tree lives under one
...
- **`repos/<org>/<repo>/` is a jj workspace of `~/stellarc/repos/<org>/<repo>/`**
...
- **`creds/` is written by the orbit**, scoped per context; an agent reads only its
  own context's credentials.

### 5.3 Workdir lifecycle is owned by the orbit

- On session creation: the orbit creates the session space `sessions/<session_id>/`
  with `.stellarc/`, `jj workspace add`s any needed origin repos into
  ...
  session space.
- On sub-delegation: the orbit creates `sessions/<main>/<sub_id>/` and
...
- **KNOWN HAZARD (must be handled):** jj has first-class conflict commits git does
  ...
  an unresolved conflict, so the agent reasons from a false premise. The orbit
...
In **both** cases: any prior session output Stellarc retained (at minimum the
...
cards:
  ...
  assignedKind: "agent"|"user"           # Stellarc is agent-native + human-in-the-loop
...
Stellarc **never continues a session in place across channels.** A session started
on Telegram, continued "in place" from Stellarc, would diverge — the Stellarc turns
would not be reflected back in Telegram, producing two inconsistent truths.
Instead, **continuing a non-Stellarc session forks it.** Forking is also the
...
| **Parallel session** (independent) | `sessions/<parallel_id>/` (own top-level space) | `jj workspace add` from the **origin repo's** commit | `forked_from` + `project_id` (Stellarc link; NOT Hermes parent) | "parallelize this project into an independent thread," sibling under the same project |
...
#### 6.6.1 How a fork is executed (pure Stellarc; zero Hermes change)

Hermes does not need to know about Stellarc forking. Verified: Hermes loads a
session to resume from **state.db** (`SessionDB.resolve_resume_session_id`), not
from a side channel. So Stellarc forks by preparing a new session row in state.db
and resuming it:

1. Stellarc writes a **new Hermes session** into state.db (WAL-safe concurrent
   write): copy the source session's messages `[0..fork_point]` into the new
   session id (the context up to the fork).
2. Inject an Stellarc marker as a system message in the new session, so the fork is
   self-describing even from raw state.db:
   `<stellarc fork="true" from_agent="<agent_id>" from_session="<src>" fork_point="<n>" stellarc_session="<new>"/>`
3. The orbit starts the agent on the new session via `AgentRuntime.start`
   (`hermes acp`, §19) — Hermes resumes it, oblivious that it is a fork; it just
   sees history + a system tag.
4. Stellarc drives the fork; the **source session is never modified** (the Telegram
   JSONL/state.db rows are untouched).

Lineage is recorded in two places: **primary** in Stellarc's event log
...
#### 6.6.3 Steering applies ONLY to Stellarc-managed sessions

Stellarc can steer/cancel/switch-model (§19) **only on sessions it manages** —
sessions it created or forked, where the orbit holds the live stdio handle. Stellarc
...
Steering a "Telegram session" therefore means: fork it into Stellarc, then steer the
fork.

> Future (post-MVP): on fork, inject a marker into the *source* channel's session
> history noting "forked into Stellarc session `<id>`", so the origin channel knows
> and can reference the fork. Out of MVP scope.

### 6.7 External-channel session sync (observe everything, own nothing)

Sessions originating outside Stellarc (Telegram, Discord, CLI, cron, api_server,
...
- **Source of truth is `~/.hermes/state.db`** (the Hermes SQLite store), NOT the
  ...
  thinner metadata. state.db runs in **WAL mode**, so Stellarc reads it
  concurrently while Hermes writes, with no lock contention.
- **Live sync:** Stellarc tails `state.db` incrementally —
  ...
  `MessageAppended` events in Stellarc's log; the UI updates live. A new
...
- Observed sessions are read-only in Stellarc. To act on one, the operator forks it
  (§6.6) into an Stellarc-managed session.
...
Stellarc declares the desired system state of each node; the orbit reconciles to
...
nodeActualState:                                 # reported by each orbit
  nodeId
...
- On startup and on a schedule, the orbit reads its applicable `nodeDesiredState`,
...
- Kubernetes-controller pattern (desired vs actual, reconcile, report). The
  control plane holds desired state; only the orbit can install software (Layer 2
...
- **Session artifacts** (`sessions/<id>/.stellarc/artifacts/`) are ephemeral agent
...
```text
~/stellarc/vaults/                              ← VAULTS (top-level resource)
└── <vault_id>/                                ← a vault
    ├── docs/                                  ← source files
...
1. Writes the proposed file to its session artifacts (`.stellarc/artifacts/proposed-doc.md`)
...
### 8.4 The vault sync engine (runs in the orbit, per node)

Each node's orbit runs a vault sync engine that maintains the local `vault.db`
...
Skills and MCP servers are Stellarc-managed and scoped. **MCP is treated as "a
skill on how to use a CLI/API"** — same management surface (library + scoped
activation).

### 9.1 Library vs activation (the core distinction)

- **Library** (`~/stellarc/skills/`, `~/stellarc/mcp/`): **all** managed skills/MCP
  definitions, materialized to disk by the orbit from the control-plane catalog.
  **Read-all**: every agent can read the library for discovery/search. Kept
  ...
  changes, the orbit re-materializes affected entries on its next reconcile tick
...
- **Activation** (which skills/MCP are *active* for a scope): configured in the
  ...
  (control plane)  (control plane)   projects/<id>/     sessions/.../.stellarc/
                                     skills.json|mcp.json  skills.json|mcp.json
  ```

  More specific overrides broader. The orbit materializes the effective set into
  the card/session `.stellarc/skills.json` and `mcp.json` at spawn.
...
skills:                                  # catalog (library entries)
  _id, name, description, scopeHint, version, contentRef   # bytes materialized to ~/stellarc/skills/
...
These are projections of the event log (§2.4); the orbit mutates them only via the
...
2. The **scheduler** (§10.5) atomically assigns it to a node with the required
   ...
   `availableSlots--`).
3. The target orbit (subscribed to its assigned commands) **claims** it atomically
   (`status: "claimed"`, sets `claimedBy`, `leaseExpiresAt`).
4. The orbit transitions it to `running` and executes the host effect (spawn
   process / open PTY / read fs).
5. The orbit reports terminal status (`completed`/`failed`/`cancelled`) with
   ...
   mutation, never by the orbit directly, so the counter cannot leak.
...
- **Host agents** (Claude Code, Codex, Hermes on a node): the orbit captures
...
a client (UI over WSS, or an orbit over its Transport) registering interest in a
...
- **Interrupt/cancel (single mechanism):** the operator/orchestrator mutation sets
  ...
  kind. The orbit (subscribed to its claimed command) stops the run: SIGTERM then
  SIGKILL after a grace period for a process agent, or the gateway abort API for a
  service agent (Hermes). The orbit then writes terminal status `cancelled`, which
...
# claim: assigned → claimed   (orbit request, validated by scheduler)
claimCommand:
...
# renew: keep the lease alive while running   (orbit, periodic)
renewLease:
  precondition: claimedBy==callerNodeId AND claimEpoch matches
  set leaseExpiresAt = now + LEASE_MS

# complete: claimed/running → terminal   (orbit request, validated by scheduler)
completeCommand:
...
|---|---|---|---|
| `nodes.lastHeartbeat` | every `HEARTBEAT_MS` (default 10 s) | orbit (`heartbeat`) | "this node is alive" |
| `hostCommands.leaseExpiresAt` | renewed every `LEASE_RENEW_MS` (default 20 s); window `LEASE_MS` (default 60 s) | orbit (`renewLease`) | "this command is still being worked" |
...
> and optional export seam. Product/audit truth remains in `stellarc.db`.
...
Stellarc needs operator-grade backend observability — a Convex-dashboard-equivalent
...
```
Stellarc control plane (single Rust binary)
...
**Invariant: every process is disposable; only the orbit daemon (per host) and the
control-plane process are durable, and the orbit must be stateless across
...
- **Orbit daemon** (one durable host process): holds NO cross-command cache. State
...
- **Worker processes:** spawned per session, killed on session end. The orbit
...
- **Control plane / orbit:** own-process RSS (+ per-view byte accounting for the
  control plane).
- **Spawned agents (incl. children):** each agent runs in its own cgroup; the
  orbit reads `memory.current` (exact, incl. children), `memory.peak`,
  `memory.max` (hard cap → kernel OOM-kill), `memory.high` (soft cap → throttle +
  reclaim), `memory.events` (OOM/throttle counter, so the orbit knows an agent was
  killed for memory and reports "card failed: OOM" rather than a mystery exit).
- The orbit polls these on its heartbeat tick and reports them up; the control
  plane aggregates into a memory view (per-node, per-agent, per-session, plus
  control-plane + orbit self-RSS) surfaced in the UI.
...
```text
orbit daemon            → unsandboxed (it IS the host supervisor)
...
|---|---|---|---|
| OS / orbit | memory, CPU per worker process | cgroup v2 (`memory.max`) / ulimit | per session |
...
The orbit owns the host's port space:

- **Sandboxed (bwrap `--unshare-net`):** each session gets a private network
  namespace; the agent binds freely on its own loopback; the orbit forwards a
  reserved host-port block in. Worker egress is **default-deny with a per-context
  allowlist** (package registries) forwarded by the orbit; no model APIs, no
  GitHub, no control-plane access for workers.
- **Host-direct:** the orbit allocates a reserved host-port block and tells the
  agent which ports to use.

Port allocation is per-node state in the orbit (mirrored to the control plane for
...
- **The control plane is the recording layer, not the routing layer** for local
  trees: the orbit mirrors SDK-streamed output to the control plane for
...
- **Black-box CLI (Claude Code/Codex):** the orbit relays via the SDK's steering
...
- **Signal/park/resume verified end-to-end** and maps *exactly* onto Stellarc's
  `agent-run`: task enqueues the host command → workflow durably parks at
  `AwaitSignal` (position persisted, survives crash) → the orbit reports
...
**tracing/journal audit layer**. NOT the distributed `PooledWorker` mode — Stellarc
...
the orbit reports completion. The workflow is the conductor; agents are the
musicians.

**"Resume a session" means spawn-fresh-and-reload, not literal process resume.**
Because processes are disposable (§11), resuming a card's session is: the orbit
...
- Host agents: the orbit parses the CLI's usage metadata (Claude Code/Codex report
...
the context; the orbit injects that key into the worker's env. When one is
...
- **Workers PRODUCE** artifacts: write files into their card worktree
  (`sessions/<session_id>/<card_id>/.stellarc/artifacts/`). Just filesystem; no
  network.
...
- **Ephemeral** (session/card-scoped prototypes): the orbit runs a static file
...
`~/stellarc/sessions/<session_id>/` grows unbounded without a lifecycle:
...
- The orbit runs cleanup on a schedule driven by control-plane state, so node disks
  do not fill.

---

## 18. SSH / terminal interface from Stellarc to each node

The operator must be able to open an interactive shell on any node from the UI.

### 18.1 Mechanism

- The orbit spawns an interactive shell via a **PTY** (`Bun.Terminal` if orbit is
  Bun; a Rust PTY crate such as `portable-pty` if orbit is Rust) — or `ssh
...
The orbit already owns PTYs (Layer 2), so terminal bridging is an extension of its
...
## 19. The AgentRuntime boundary (orbit command queue + per-harness stdio adapter)

The orbit holds, per managed session, the **process handle** (PID + lifecycle)
AND the **stdio control channel** to the spawned harness. Stellarc does not reach
into an agent's memory; it sends a command to the orbit, and the orbit's
...
```ts
// Orbit owns a COMMAND QUEUE per managed session. Stellarc enqueues; the orbit
...
`events`) run over the **same long-lived stdio channel** the orbit holds. Because
the orbit owns the channel, it does the steer/cancel call itself — no file, no
...
This boundary lives in the orbit (Layer 2): the orbit holds the PID + stdio
...
Stellarc **accepts node death**; it does not fight it with live migration.
...
- **Accepted limitation (state in user-facing docs):** Stellarc guarantees state
...
- **Hermes gateway socket / `/api/pty` as the bridge.** The gateway's only
  ...
  structured gateway endpoint would couple Stellarc to the gateway's lifecycle.
  Rejected in favor of driving `hermes acp` (ACP over stdio) directly from the
  orbit (§19), which already exposes structured prompt/steer/cancel/streaming.
- **File-based steer drain (orbit writes a `.stellarc/steer` file the agent polls).**
  Unnecessary once ACP is the control lane: the orbit owns the stdio channel and
...
- **Continuing an external-channel session in place.** Produces cross-channel
  divergence (Stellarc turns absent from the origin channel). Rejected in favor of
  always forking a non-Stellarc session (§6.6).
...
- **Control-plane-only (no orbit).** The control plane is one authority over many
  hosts; host effects are inherently per-host. The orbit is a hard requirement.
...
- Stellarc is fleet infrastructure, not an agent. Hermes and other agents are
  managed, not reimplemented.
- The control plane is a self-contained, shippable Rust binary; the orbit is the
  single durable host-level process and trust boundary per host. Everything else
  is disposable.
- Stellarc owning the workdir eliminates the Hermes Studio cross-contamination bug.
