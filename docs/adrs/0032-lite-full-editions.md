...
- Amends: ADR 0031 (tier ↔ edition mapping, §3 below), ADR 0008 (envoy transport gains TCP-loopback listener), ADR 0014 (edge also serves `olympus.localhost` app routing)
...
Olympus ships as two editions of one codebase, split by a storage/identity seam — never a fork.

| | **Lite** | **Full** |
|---|---|---|
| Install | desktop app (Tauri shell bundling Hall+Envoy+UI); user-tier per ADR 0031 | Linux host installer; root at install, never at runtime; system-tier per ADR 0031 |
| Hall storage | SQLite | Postgres + pgvector |
...
| Agent runner | Envoy | Envoy (never act_runner) |
...
| Web UI | Hall serves SPA; LAN-exposable for mobile browsers | same |
| Mobile app | connects to any external Hall (lite or full); separate track | same |

Multi-node ≠ multi-user: lite allows N envoy nodes attached to one single-user Hall. The lite/full line is drawn at identity (second human → full), never at node count.
...
| Full | system unit, unprivileged `olympus-envoy` daemon | systemd transient units via D-Bus: per-session Unix account (0031), `MemoryMax=`/`CPUQuota=` cgroups |
...
WSL nodes: desktop app auto-installs envoy into the WSL distro and attaches over TCP-loopback (WSL2 localhost forwarding). No iroh required on the same machine.

## 4. Sessions: manifest, workspaces, artifacts

**Session manifest** (Hall-owned record, Envoy-materialized at `ensure_runtime`): `repos[{slug,rev}]`, `vaults[{slug,mode}]`, `context[]` (0028), `setup` (existing table). Fork copies the manifest. Project-level saved manifests are workspace templates (`project.json`, 0026 §6).
...
4. Envoy cleanup sweep (heartbeat loop): workspace of a completed/handed-over session older than N days → delete; workspace with no Hall session row → delete. Durable refs (pushed bookmarks/branches) are the only thing that survives teardown.
...
**Session authority matrix** (field → owner) is a prerequisite deliverable of the seam extraction, in 0026 §4 style: Hall log | Hermes state.db | envoy spool | workspace. No session field may acquire two writable owners.
...
- `olympus.localhost` (RFC 6761, resolves loopback everywhere, zero DNS setup) is the one hostname. Hall-side proxy owns it: `/` → Hall SPA, `/app/<slug>` → managed app upstream over iroh. Path routing first; per-app subdomains only when an app's root-absolute assets break.
- `resolve(name) → NodeId` is one shared registry lookup (Hall node list). Consumers: the HTTP proxy and the SSH connector.
- SSH to nodes: `ProxyCommand olympus-iroh-connect %h` — one small binary opening one iroh stream, stdio-piped. No tunnel daemon, no listener, no MagicDNS.
...
2. Session manifest + Envoy materializer + workspace cleanup sweep (sweep first has a live customer: fxcompute at 95% disk).
3. TCP-loopback listener + WSL envoy auto-install.
...
- **Auth proxy for SSO** — rejected outright; Forgejo OIDC native, Hall per 0010/0024.
...
- **act_runner as agent runner** — different lifecycle/scheduler; Envoy owns agents.
