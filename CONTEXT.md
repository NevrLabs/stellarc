...
- **Kernel** — the irreducible privileged core: plugin loader, capability check, event log (append/read/subscribe + schemas), identity/ACL enforcement, wire transit (HTTP/MCP/WS chokepoints). Resource kinds are opt-in: arcdrive (docs/blob store), sessions, tables, nodes etc. ship as first-party plugins, not kernel tables. Allowlist config is kernel-read-only. Upgraded only by CP redeploy.
...
- **Arcdrive** — resource plugin: docs/blob store (markdown vault in the Obsidian sense), org document store. Careful-implementation area: plane-stored AND node-synced — ownership/merge semantics get explicit design in its own ticket. NOT secrets — secret management is external (openbao/HashiCorp Vault/Bitwarden); at most a request-injector plugin later.
- **Harness config module** — syncs harness configuration across nodes, including harness auth; API keys node-held or routed via the builtin aiproxy (aigw, recommended).
- **Workflow (durability)** — durable-by-construction: steps checkpoint as events, resume by replay after restart; side effects only via kernel-dispatched logged actions; at-least-once + idempotency keys on `session:drive`. Core subsystem — engine choice (DBOS-style own-log / Temporal / BullMQ / EffectMQ) deferred to its own design ticket.
- **Desired state** — config is plane-declared per-org; nodes reconcile and report actual as claims; drift = flagged diff. Node overrides declared to the plane, never around it. Auth values never in desired state — references only.
- **Template** — versioned, org-scoped, instantiable bundle (agent/project/workflow) [ticket #9].
- **Event namespacing** — every event type is `pluginId:type` with owning-plugin provenance; replay is tolerant (unknown/uninstalled namespaces kept as bytes, skipped by projections); namespace purge is an explicit operator command. Envelope carries `schemaVersion`; owning plugin ships a pure, total upcaster chain — readers see latest shape only; un-upcastable events are quarantine-grade (D13), never dropped.
...
- **D19 (schema evolution)** — reader-side upcasting: schemaVersion on envelope, owner-shipped pure/total upcaster chain, raw log forever; un-upcastable = quarantine, not drop.
- **D18 (workflow durability)** — durable engine: event-checkpointed steps, replay-resume across boot generations, at-least-once + idempotency keys. Semantics are doctrine; engine implementation deferred to dedicated design.
- **D17 (config direction)** — plane-declared desired state, node reconciles, actual reported as claim; drift flagged; overrides declared to plane; auth by reference only. Docs/blob store named arcdrive.
- **D16 (secrets/terminology)** — 'vault' = markdown vault → renamed arcdrive (via D17); secret management external (openbao/HCV/Bitwarden), injector plugin deferred; harness keys node-held or via builtin aiproxy (aigw); harness config module syncs cross-node incl. auth.
...
- **D6 (kernel boundary)** — kernel = boot + loader + capability check + event log + authn/z + transit chokepoints. Resources opt-in (plugin-provided): arcdrive, sessions, tables, nodes, harnesses, grants-as-resource. Auth plugin non-removable at runtime; CP refuses plugin routes without it. TCB = kernel; upgrades by redeploy only.
