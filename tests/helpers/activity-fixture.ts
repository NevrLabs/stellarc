...
export const EXPECTED_CONSTRAINTS: Array<{
  ...
  { table: "user_notification_org_rule", name: "unor_user_org_unique", type: "u" },
  { table: "user_notification_org_rule", name: "unor_org_id_unique", type: "u" },
  {
...
export async function assertConstraints(sql: Sql): Promise<void> {
  ...
  const have = new Set(rows.map((r) => `${r.table_name}:${r.conname}:${r.contype}`));
