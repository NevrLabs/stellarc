...
type SourceRow = { org: string; id: string; slug: string; name: string; ticket_max: number };

const SOURCE_BOARDS: SourceRow[] = [
	{ org: "org-1", id: "sb-1", slug: "imported", name: "Imported Board", ticket_max: 3 },
	{ org: "org-2", id: "sb-2", slug: "other", name: "Other Board", ticket_max: 0 },
];
...
test("T18: import restores boards with exact column fidelity and PK-set equality", async () => {
	...
	const destIds = (await sql`SELECT id FROM "board" WHERE id IN ('sb-1','sb-2')`).map((r) => r.id);
	...
	const statuses = await sql`SELECT slug FROM "column" WHERE board_id = 'sb-1' ORDER BY position`;
...
test("T19: identical rerun produces zero new events and identical ledger", async () => {
	const before = (await sql`SELECT count(*)::int AS count FROM event WHERE org = 'org-1'`)[0].count;
	const ledgerBefore = (await sql`SELECT digest FROM work_import WHERE source_id = 'test-source' ORDER BY table_name`).map((r) => r.digest);
	const { importWork } = await import("../../packages/domain/src/work-import");
	const report = await importWork(sql, "test-source", {
		boards: SOURCE_BOARDS.map((b) => ({
			organization_id: b.org,
	...
	const after = (await sql`SELECT count(*)::int AS count FROM event WHERE org = 'org-1'`)[0].count;
	expect(after).toBe(before);
	const ledgerAfter = (await sql`SELECT digest FROM work_import WHERE source_id = 'test-source' ORDER BY table_name`).map((r) => r.digest);
...
test("T20: preflight aborts the whole run before any write on duplicate slug", async () => {
	...
	const report = await importWork(sql, "test-source-2", {
		boards: [
			{ organization_id: "org-1", id: "sb-dup", slug: "IMPORTED", name: "Dup", last_task_number: 0 },
			{ organization_id: "org-2", id: "sb-clean", slug: "clean", name: "Clean", last_task_number: 0 },
		],
	});
	expect(report.aborted).toBe(true);
	expect(report.errors.some((e: string) => e.toLowerCase().includes("slug"))).toBe(true);
	// NOTHING was written — not even the clean board (single-transaction run).
	const [written] = await sql`SELECT count(*)::int AS count FROM "board" WHERE id = 'sb-clean'`;
	expect(written.count).toBe(0);
	const [dup] = await sql`SELECT count(*)::int AS count FROM "board" WHERE id = 'sb-dup'`;
	expect(dup.count).toBe(0);
});

test("T18: ledger rows carry per-table digests (identity_import pattern)", async () => {
	const rows = await sql`SELECT table_name, digest FROM work_import WHERE source_id = 'test-source' ORDER BY table_name`;
	expect(rows.length).toBeGreaterThan(0);
	for (const row of rows) {
		expect(row.digest).toMatch(/^[a-f0-9]{64}$/);
	}
	const { createHash } = await import("node:crypto");
	expect(createHash("sha256").update("x").digest("hex")).toMatch(/^[a-f0-9]{64}$/);
});
