//! Stellarc Orbit — the runtime-holder binary (ADR 0008 S3).
//!
//! Connects to Axis's UDS socket, registers (hello with protocolVersion +
//! BuildVersion + discovered agents), heartbeats, and dispatches Axis session
//! frames (`ensure_runtime`, `prompt`, `steer`, `cancel`, `stop`,
//! `respond_permission`, `probe`) to a [`RuntimeTable`]. Agent events are
//! streamed back as `OrbitFrame::Event` frames with per-session monotonic `seq`.
//!
...
//!
//! Usage:
//!   stellarc-orbit [--socket <path>] [--node-id <id>] [--mock]
//! Defaults: socket = `$STELLARC_CONTROL_SOCKET` or `$STELLARC_HOME/control.sock`.
...
pub async fn run() -> Result<()> {
    if flag_present("--version") || flag_present("-V") {
        println!("stellarc-orbit {}", env!("CARGO_PKG_VERSION"));
        return Ok(());
    }
    ...
    // bootstrap installer to register the id with the Axis's allowlist BEFORE
    ...
    tracing::info!(socket = %socket.display(), node = %node_id, mock, "stellarc-orbit starting");
    ...
    // Reconnect loops below: Axis restarts must NOT kill this orbit
    ...
    let job_root = arg_value("--job-root")
        .or_else(|| std::env::var("STELLARC_JOB_ROOT").ok())
        .map(PathBuf::from)
    ...
    if let Some(state_db) = observer_state_db()? {
        ...
        std::thread::Builder::new()
            .name("stellarc-statedb-observer".into())
        ...
        loop {
            match crate::transport::connect_to_axis(&endpoint, target).await {
                Ok((send, recv)) => {
                    ...
                    tracing::warn!("Axis iroh connection lost; reconnecting…");
                }
                Err(e) => {
    ...
    tracing::info!("connected to Axis UDS");
    ...
    loop {
        let s = match stream.take() {
            ...
            None => {
                ...
                tracing::info!("reconnected to Axis UDS");
                s
            }
        };
        ...
        tracing::warn!("Axis connection lost; reconnecting…");
        tokio::time::sleep(std::time::Duration::from_millis(500)).await;
    }
}

/// Root for this Orbit's state and node-local resources. Development and
/// production must never share this tree.
fn stellarc_home() -> Result<PathBuf> {
    if let Ok(home) = std::env::var("STELLARC_HOME") {
...
/// Per-orbit state dir (iroh key lives here):
/// `$STELLARC_HOME/orbit/<node-id>/`.
fn orbit_state_dir(node_id: &str) -> Result<PathBuf> {
    validate_resource_component("node id", node_id)?;
    Ok(stellarc_home()?.join("orbit").join(node_id))
}
...
struct Conn {
    ...
    /// driven solely by AxisFrame::Terminal* frames.
    pty: Arc<crate::pty::PtyManager>,
    hello: Mutex<Option<OrbitFrame>>,
...
impl Conn {
    ...
    async fn send_frame(&self, frame: &OrbitFrame) -> Result<()> {
        let json = serde_json::to_string(frame).context("serializing orbit frame")?;
    ...
    W: tokio::io::AsyncWrite + Send + Unpin + 'static,
{
    ...
    // into an mpsc that a forwarder task drains into the Axis connection.
    ...
    // Forward PTY output/exit frames to Axis over this connection.
    let pty_fwd_handle = {
    ...
    for session_id in conn.spool.sessions()? {
        ...
        runtimes.push(stellarc_proto::frames::RuntimeStatus {
    ...
    for attempt in &mut job_attempts {
        let identity = crate::job_table::wire_id(&attempt.job_id, attempt.attempt_epoch);
        if let Some(OrbitFrame::JobResult {
            seq,
            exit_code,
            truncated,
            timed_out,
        ...
        {
            attempt.state = stellarc_proto::frames::JobAttemptState::Completed;
    ...
    *conn.hello.lock().await = Some(hello);
    ...
    let replay_handle = {
        ...
        tokio::spawn(async move {
            // Send each frame at most once per live connection unless Axis
    ...
    // processes on resource-constrained orbits.
    let reaper_handle = {
    ...
    // Read loop — dispatch AxisFrames.
    let mut lines = BufReader::new(reader).lines();
    loop {
        let line = match lines.next_line().await {
            Ok(Some(l)) => l,
            Ok(None) => {
                tracing::info!("Axis disconnected");
                break;
            }
            Err(e) => {
        ...
        let frame: AxisFrame = match serde_json::from_str(line) {
            Ok(f) => f,
            Err(e) => {
                tracing::warn!(error = %e, "unparseable AxisFrame, skipping");
                continue;
            }
        };
        ...
        // socket read loop: Axis sends ACKs on that same full-duplex stream.
        // Replaying inline can fill both socket buffers (Orbit waits to write
        // replay, Axis waits to write ACK) and starve heartbeats indefinitely.
    ...
    // Read loop exited (Axis disconnected or error). Abort the heartbeat and
...
fn frame_sequence(frame: &OrbitFrame) -> Option<u64> {
    match frame {
        ...
        | OrbitFrame::Observed { seq, .. }
        | OrbitFrame::JobOutput { seq, .. }
        | OrbitFrame::JobResult { seq, .. } => Some(*seq),
        _ => None,
    }
}
...
async fn dispatch_frame(conn: Arc<Conn>, frame: AxisFrame) -> Result<()> {
    match frame {
        ...
        AxisFrame::ReRegister => {
            tracing::warn!("Axis requested re-registration");
            conn.unacked_heartbeats
                .store(0, std::sync::atomic::Ordering::SeqCst);
            conn.send_hello().await?;
        }
        ...
        AxisFrame::Cancel { req_id, session_id } => {
        ...
        AxisFrame::Stop { req_id, session_id } => {
        ...
        AxisFrame::Ack { session_id, seq } => {
            conn.spool.acknowledge(&session_id, seq)?;
            tracing::debug!(session = %session_id, seq, "ack from Axis truncated spool");
        }
        AxisFrame::ResumeFrom { session_id, seq } => {
            // u64::MAX is the wire sentinel for "Axis has no watermark yet";
            ...
            tracing::debug!(session = %session_id, seq, "replayed spool from Axis watermark");
        }
        ...
        AxisFrame::TerminalClose { terminal_id } => {
...
async fn send_and_stream(conn: &Conn, session_id: &str, cmd: AgentCommand) -> Result<()> {
    ...
    // Drain the event stream, forwarding each as an OrbitFrame::Event.
    ...
    while let Some(event) = events.next().await {
        ...
        let mut frame = OrbitFrame::Event {
...
fn run_observer(path: PathBuf, spool: Arc<EventSpool>) {
    let mut observer = match crate::observer::StateDbObserver::open(&path) {
            ...
            Ok(events) => {
                ...
                for payload in events {
                    let hermes_id = match &payload {
                        stellarc_proto::frames::ObservedEvent::Session { hermes_id, .. }
                        | stellarc_proto::frames::ObservedEvent::Message { hermes_id, .. } => {
                            hermes_id
                        }
                    };
                    let session_id = format!("observed:{hermes_id}");
                    let mut frame = OrbitFrame::Observed {
                        seq: 0,
                        session_id,
                        payload,
                    };
...
/// Resolve the Axis UDS socket path: `--socket` arg → `STELLARC_CONTROL_SOCKET`
/// → `$STELLARC_HOME/control.sock`.
fn resolve_socket() -> Result<PathBuf> {
    if let Some(p) = arg_value("--socket") {
        return Ok(PathBuf::from(p));
    }
    if let Ok(p) = std::env::var("STELLARC_CONTROL_SOCKET") {
        return Ok(PathBuf::from(p));
    }
    Ok(stellarc_home()?.join("control.sock"))
}
...
fn production_factory() -> RuntimeTable {
    ...
    RuntimeTable::with_factory(Arc::new(|session_id, spec: &RuntimeSpec| {
        let cwd = if let Some(session_id) = session_id {
            ...
            let workspace =
                materialize_session_space(&stellarc_home()?, organization_id, session_id)?;
            tracing::info!(
                session = session_id,
        ...
        let model_set_style =
            crate::bridge::hermes::model_set_style_for_agent(spec.agent.as_deref());
...
fn configured_roles() -> Result<Vec<NodeRole>> {
    let raw = arg_value("--roles").or_else(|| std::env::var("STELLARC_ORBIT_ROLES").ok());
    // Every orbit hosts operator terminals by default (ADR 0021 cockpit) — the
    ...
    if let Some(raw) = raw {
            ...
            .filter(|value| !value.is_empty())
        {
            match role {
                ...
                other => anyhow::bail!("unknown orbit role: {other}"),
            }
        }
    }
...
mod tests {
    ...
    fn materializes_private_node_local_session_space() {
        ...
        let root = temp.path().join("stellarc-dev");
        let session =
    ...
    async fn reregister_frame_causes_a_second_hello_on_same_connection() {
        ...
        let (orbit, axis) = tokio::io::duplex(16 * 1024);
        let (orbit_reader, orbit_writer) = tokio::io::split(orbit);
        let (axis_reader, mut axis_writer) = tokio::io::split(axis);
        ...
        let mut lines = tokio::io::BufReader::new(axis_reader).lines();
        assert!(matches!(
            serde_json::from_str::<OrbitFrame>(&lines.next_line().await.unwrap().unwrap()).unwrap(),
            OrbitFrame::Hello { .. }
        ));
        axis_writer
            .write_all(
                format!(
                    "{}\n",
                    serde_json::to_string(&AxisFrame::ReRegister).unwrap()
                )
                .as_bytes(),
            )
            .await
            .unwrap();
        assert!(matches!(
            serde_json::from_str::<OrbitFrame>(
                &tokio::time::timeout(std::time::Duration::from_secs(1), lines.next_line())
                    .await
                    .expect("orbit must re-Hello immediately")
                    .unwrap()
                    .unwrap()
            )
            .unwrap(),
    ...
    async fn three_missed_heartbeat_acks_cause_a_fresh_hello() {
        ...
        let mut lines = tokio::io::BufReader::new(axis_reader).lines();
        let first: OrbitFrame =
            serde_json::from_str(&lines.next_line().await.unwrap().unwrap()).unwrap();
        assert!(matches!(first, OrbitFrame::Hello { .. }));
        ...
        for _ in 0..HEARTBEAT_REREGISTER_AFTER {
            ...
            let frame: OrbitFrame =
                serde_json::from_str(&lines.next_line().await.unwrap().unwrap()).unwrap();
            assert!(matches!(frame, OrbitFrame::Heartbeat { .. }));
        }
        let repaired: OrbitFrame =
            serde_json::from_str(&lines.next_line().await.unwrap().unwrap()).unwrap();
        assert!(matches!(repaired, OrbitFrame::Hello { .. }));
        task.abort();
    }

    #[test]
    fn resume_replay_never_blocks_the_socket_read_loop() {
        assert!(!dispatch_inline(&AxisFrame::ResumeFrom {
            session_id: "session".into(),
            seq: u64::MAX,
        }));
        assert!(dispatch_inline(&AxisFrame::Ack {
            session_id: "session".into(),
            seq: 42,
        }));
    }
    ...
    fn periodic_replay_advances_once_per_connection() {
        ...
        // Explicit Axis ResumeFrom bypasses this periodic cursor entirely.
