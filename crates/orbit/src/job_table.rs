//! Durable, fenced shell job execution for the Orbit (ADR 0017 §5).
        ...
        for attempt in attempts.values_mut() {
            if attempt.status.state == JobAttemptState::Running {
                ...
                attempt.status.terminal_reason =
                    Some("orbit_restarted_while_attempt_running".into());
                attempt.pgid = None;
...
fn gated_command(
    ...
    // The child waits for one byte before exec. If Orbit dies before the PGID
...
fn resolve_cwd(root: &Path, cwd: Option<&str>) -> Result<PathBuf> {
    ...
    {
        anyhow::bail!("cwd must stay within the orbit job workspace root");
    }
...
mod tests {
    ...
    async fn launch_gate_prevents_effects_before_pgid_is_durable() {
        ...
        drop(child.stdin.take()); // simulate Orbit crash before ledger fsync
    ...
    async fn duplicate_dispatch_is_idempotent_and_drains_before_result() {
        ...
        loop {
            ...
            match pending.frame {
                OrbitFrame::JobOutput { data, .. } => output.push_str(&data),
                OrbitFrame::JobResult { exit_code, .. } => {
        ...
        let result = tokio::time::timeout(Duration::from_secs(3), async {
            loop {
                if let Some(pending) = rx.recv().await {
                    if let OrbitFrame::JobResult { timed_out, .. } = pending.frame {
    ...
    fn restart_kills_persisted_running_process_group() {
        ...
        assert!(exited, "persisted process group survived orbit restart");
    }
