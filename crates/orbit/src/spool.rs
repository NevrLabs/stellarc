//! Transactional Orbit execution event outbox (ADR 0033).
//!
//! Orbit owns active session/job-run history. Events and sequence allocation
//! live in one SQLite transaction; Axis acknowledgements remove delivered
...
impl EventSpool {
    ...
    pub fn with_cap(state_dir: &Path, cap: u64) -> Result<Self> {
        ...
        let db = Connection::open(state_dir.join("orbit.sqlite"))?;
    ...
    pub fn append_next(&self, frame: &mut OrbitFrame) -> Result<u64> {
    ...
    pub fn append(&self, frame: &OrbitFrame) -> Result<()> {
    ...
    pub fn read(&self, session_id: &str, after: Option<u64>) -> Result<Vec<OrbitFrame>> {
        let db = self.db.lock().expect("event store mutex poisoned");
...
fn insert_frame(db: &Connection, frame: &OrbitFrame, cap: u64) -> Result<()> {
    let (session_id, seq) = event_identity(frame).context("only event frames are spoolable")?;
...
fn event_identity(frame: &OrbitFrame) -> Option<(&str, u64)> {
    match frame {
        OrbitFrame::Event {
            session_id, seq, ..
        }
        ...
        OrbitFrame::JobOutput { job_id, seq, .. } | OrbitFrame::JobResult { job_id, seq, .. } => {
            Some((job_id, *seq))
        }
        _ => None,
    }
...
fn set_event_seq(frame: &mut OrbitFrame, seq: u64) -> Option<()> {
    match frame {
        ...
        | OrbitFrame::Observed { seq: value, .. }
        | OrbitFrame::JobOutput { seq: value, .. }
        | OrbitFrame::JobResult { seq: value, .. } => {
            *value = seq;
            Some(())
        }
        _ => None,
...
mod tests {
    use super::*;
    use stellarc_proto::agent::AgentEvent;

    fn event(session: &str, seq: u64) -> OrbitFrame {
        OrbitFrame::Event {
    ...
    fn stores_events_in_one_transactional_sqlite_database() {
        ...
        assert!(dir.path().join("orbit.sqlite").is_file());
        assert!(!dir.path().join("spool").exists());
        let db = Connection::open(dir.path().join("orbit.sqlite")).unwrap();
