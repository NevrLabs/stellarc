//! Stellarc agent bridge — uniform command queue → real agent runtime.
//!
//! The bridge is the seam between Stellarc's session model and an external agent
//! runtime (Hermes via ACP). Stellarc issues high-level [`AgentCommand`]s and
//! consumes [`AgentEvent`]s; the concrete runtime (e.g. [`HermesAgentRuntime`])
//! maps those onto the real wire protocol.
//!
//! See `docs/plans/2026-06-28-stellarc-mvp.md` Task 4.1 and
...
// `stellarc-proto` (ADR 0008 — proto is the only crate Axis and Orbit share).
// Re-exported here so existing call sites keep working unchanged.
pub use stellarc_proto::{AgentCommand, AgentEvent, PermissionOption};
...
mod tests {
    ...
    fn set_model_uses_set_model_for_hermes_style() {
        use stellarc_proto::ModelSetStyle;
