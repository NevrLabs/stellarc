...
use stellarc_proto::{AcpFraming, ModelSetStyle};
        ...
        tokio::spawn(async move {
            ...
            loop {
                match reader.read(&mut chunk).await {
                    ...
                    Err(error) => {
                        debug!(target: "stellarc.bridge.client", %error, "ACP read failed");
                ...
                loop {
                    match codec.decode(&mut buffer) {
                        ...
                        Ok(None) => break,
                        Err(error) => {
                            debug!(target: "stellarc.bridge.client", %error, "ACP frame decode failed");
...
async fn handle_message(
    ...
    // correlated handshake response. It is already in Stellarc's event log.
...
pub fn build_initialize_request(id: AcpId) -> AcpRequest {
    AcpRequest {
        ...
        params: json!({
            ...
            "clientInfo": { "name": "stellarc-axis", "version": env!("CARGO_PKG_VERSION") }
        }),
    }
}
