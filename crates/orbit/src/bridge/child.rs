...
//!
//! This is intentionally the only Orbit bridge module that imports
...
fn claude_acp_binary() -> String {
    if let Ok(path) = std::env::var("STELLARC_CLAUDE_ACP_BIN") {
        return path;
    }
    let base = std::env::var("STELLARC_HOME")
        .map(std::path::PathBuf::from)
        .or_else(|_| {
            std::env::var("HOME").map(|home| std::path::PathBuf::from(home).join(".stellarc"))
        })
        .unwrap_or_else(|_| std::path::PathBuf::from(".stellarc"));
    base.join("adapters")
