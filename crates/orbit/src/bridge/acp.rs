...
//! `docs/plans/2026-06-28-stellarc-mvp.md` Task 4.1 for the method table.

use serde::{Deserialize, Serialize};
use serde_json::{json, Value};

use super::{AgentCommand, AgentEvent};
pub use stellarc_proto::ModelSetStyle;
...
/// round-trip, but the constructor is integer-typed for Stellarc's own requests.
...
/// now lives in `stellarc-proto` (ADR 0008) and inherent impls on foreign types
...
impl AgentEventAcpExt for AgentEvent {
    ...
    /// - `user_message_chunk`   → ignored (Stellarc echoes its own prompts)
    /// - `available_commands_update` → ignored
    ///
    /// Returns `None` for notifications Stellarc does not surface (the caller
    /// drops them silently).
    fn from_notification(notif: &AcpNotification) -> Option<Self> {
    ...
    /// Map an incoming agent→client **request** into an [`AgentEvent`].
    ///
    /// The only request Stellarc surfaces is `session/request_permission` (the
