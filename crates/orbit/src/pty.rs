//! Operator terminal (PTY) subsystem for the Orbit (ADR 0021 cockpit).
//!
//! A `PtyManager` owns a set of live operator shells keyed by Axis-issued
//! `terminal_id`. Each shell is `$SHELL` running inside a **tmux session**
//! (`stellarc-term-<id>`). Tmux provides persistence: the shell survives a
...
//! the manager is driven exclusively by `AxisFrame::Terminal*` frames, which
//! Axis only emits from the operator-authenticated cockpit WebSocket.
//!
//! Bytes are opaque (not guaranteed UTF-8) and cross the Axis wire base64.
...
pub fn tmux_session_name(terminal_id: &str) -> String {
    format!("stellarc-term-{}", terminal_id)
}
...
/// What the manager needs to emit output/exit frames back toward Axis.
...
/// `OrbitFrame::TerminalOutput` / `TerminalExited`. Keeps `pty.rs` free of the
...
impl PtyManager {
    ...
    /// `stellarc-term-<id>` already exists, re-attach to it (reconnect). If the
...
mod tests {
    ...
    fn tmux_session_name_is_stable() {
        assert_eq!(tmux_session_name("abc-123"), "stellarc-term-abc-123");
        assert_eq!(tmux_session_name("x"), "stellarc-term-x");
    }
