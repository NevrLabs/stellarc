...
//! reports the session id (which should land in state.db tagged source=stellarc).

use std::time::Duration;

use stellarc_axis::bridge::hermes::{HermesAgentRuntime, HermesRuntimeConfig};
use stellarc_axis::bridge::{AgentCommand, AgentRuntime};
...
async fn main() -> anyhow::Result<()> {
    ...
    let cfg = HermesRuntimeConfig {
        ...
        mcp_servers: Vec::new(),
        env: Vec::new(),
        framing: stellarc_axis::bridge::hermes::AcpFraming::NewlineJson,
        ..Default::default()
    };
    let rt = HermesAgentRuntime::new_arc(cfg);

    eprintln!("[smoke] starting hermes acp (session/new, source=stellarc)…");
    rt.start(None).await?;
    ...
    let printer = tokio::spawn(async move {
        ...
        while let Some(ev) = events.next().await {
            eprintln!("[event] {ev:?}");
            if matches!(ev, stellarc_axis::bridge::AgentEvent::Text(_)) {
                got_text = true;
            }
            if matches!(ev, stellarc_axis::bridge::AgentEvent::Done { .. }) {
                break;
            }
        }
        got_text
