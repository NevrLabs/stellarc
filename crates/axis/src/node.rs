//! Node registry — tracks fleet nodes (orbits) connected via UDS.
//!
//! The control plane listens on a Unix domain socket
//! (`~/.stellarc/control.sock`). Each node (orbit) connects and speaks a
//! JSON-lines using the exact-v1 `OrbitFrame` contract. The first frame must be
...
//! orbit is just another node in the registry.
...
/// How a node is connected to the Axis.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum NodeTransport {
    /// In-process (the Axis's own host).
    Local,
...
pub struct NodeInfo {
    ...
    /// The node's iroh public key (set for iroh-connected orbits; used to
    /// revoke it from the allowlist on Remove).
    pub iroh_node_id: Option<String>,
    /// Agents this node's orbit has discovered on its host (Hermes profiles +
    /// installed CLI harnesses). Populated by the node's orbit — for the local
    /// node, by in-process discovery at boot and on manual refresh. Empty until
    /// a remote orbit reports its agents.
...
pub struct NodeRegistry {
    ...
    roles: Arc<RwLock<HashMap<String, Vec<stellarc_proto::frames::NodeRole>>>>,
...
impl NodeRegistry {
    ...
    pub async fn enroll(&self, node_id: &str, iroh_node_id: &str) -> anyhow::Result<()> {
        ...
        // raw iroh key because the old Axis never persisted node names. When
    ...
    /// or a remote orbit re-reporting). Returns the updated list, or an error if
    /// the node is unknown.
    ...
    pub async fn set_roles(&self, node_id: &str, roles: Vec<stellarc_proto::frames::NodeRole>) {
        self.roles.write().await.insert(node_id.to_owned(), roles);
    }

    pub async fn has_role(&self, node_id: &str, role: stellarc_proto::frames::NodeRole) -> bool {
        self.roles
            .read()
            .await
            .get(node_id)
...
/// the lifetime of the orbit — heartbeats arrive on the same socket.
///
/// Connections speak the exact-v1 `OrbitFrame` protocol. On disconnect, the
/// node is deregistered and its OrbitConnection (if any) is removed.
///
...
/// Transport-generic orbit connection handler: the same JSON-lines dispatch
    ...
    W: tokio::io::AsyncWrite + Send + Unpin + 'static,
{
    ...
    let epoch = orbit_conns.allocate_epoch();
    let (shutdown_tx, mut shutdown_rx) = tokio::sync::watch::channel(false);
    let mut connected_node: Option<String> = None;
    // The OrbitConnection is set only after an accepted exact-v1 Hello.
    let mut conn: Option<Arc<crate::server::orbit_conn::OrbitConnection>> = None;
    let mut writer: Option<crate::server::orbit_conn::BoxedWriter> = Some(Box::new(writer));
    ...
    loop {
        ...
        let frame = match serde_json::from_str::<stellarc_proto::frames::OrbitFrame>(&line) {
            Ok(frame) => frame,
            Err(error) => {
                tracing::warn!(%error, "rejecting malformed orbit frame");
                break;
            }
        };
        if conn.is_none() {
    ...
    if let Some(node_id) = connected_node {
        ...
        // Fail all pending requests on this orbit's connection and remove it.
        if let Some(conn) = orbit_conns.remove_epoch(&node_id, epoch).await {
            conn.fail_all().await;
        }
    }
}

/// Outcome of handling an OrbitFrame: Continue or Disconnect (bye).
#[derive(PartialEq)]
enum FrameOutcome {
    Continue,
    Disconnect,
...
/// Outcome of handling an OrbitFrame Hello: Accepted (with the OrbitConnection)
/// or Rejected (protocol mismatch → connection closing).
enum HelloOutcome {
    Accepted(Arc<crate::server::orbit_conn::OrbitConnection>),
    Rejected,
}
...
async fn handle_envoy_hello(
    ...
    registry: &NodeRegistry,
    orbit_conns: &crate::server::orbit_conn::OrbitConnections,
    writer: crate::server::orbit_conn::BoxedWriter,
    ...
    use stellarc_proto::version::PROTOCOL_VERSION;

    let OrbitFrame::Hello {
        node_id,
        hostname,
        slots_total,
        protocol_version,
    ...
    if protocol_version != PROTOCOL_VERSION {
        tracing::warn!(
            ...
            "rejecting orbit: protocol version mismatch"
        );
    ...
    // Parse the agents JSON into AgentInfo (best-effort; the orbit sends
    ...
    let (conn, old) = match orbit_conns
    ...
    for attempt in &job_attempts {
        ...
        if let Err(error) = conn
            .send_request(stellarc_proto::frames::AxisFrame::ResumeFrom {
                session_id: identity,
                seq: watermark,
            })
            .await
    ...
    for runtime in runtimes {
        let watermark = match conn.watermark(&runtime.session_id) {
            ...
            Ok(None) => u64::MAX,
            Err(error) => {
                tracing::error!(session = %runtime.session_id, error = %error, "reading orbit replay watermark");
                continue;
            }
        };
        if let Err(error) = conn
            ...
            .await
        {
            tracing::error!(error = %error, "requesting orbit spool replay");
        }
    }
...
async fn handle_envoy_frame(
    ...
    registry: &NodeRegistry,
    orbit_conns: &crate::server::orbit_conn::OrbitConnections,
    conn: &mut Option<Arc<crate::server::orbit_conn::OrbitConnection>>,
    transport: NodeTransport,
    peer_iroh_id: Option<String>,
    epoch: u64,
) -> FrameOutcome {
    ...
    match frame {
        ...
        OrbitFrame::Heartbeat {
            ...
            let reply = match registry.heartbeat(&node_id, slots_used, Some(epoch)).await {
                Ok(()) => stellarc_proto::frames::AxisFrame::HeartbeatAck,
                Err(e) if !registry.iroh_peer_is_allowlisted(peer_iroh_id.as_deref()) => {
                    tracing::warn!(node = %node_id, error = %e, "closing unrepairable orbit heartbeat");
                    return FrameOutcome::Disconnect;
                }
                Err(e) => {
                    tracing::warn!(node = %node_id, error = %e, "heartbeat registration mismatch; requesting registration");
                    stellarc_proto::frames::AxisFrame::ReRegister
                }
            };
        ...
        OrbitFrame::Bye { node_id } => {
            tracing::info!(node = %node_id, "orbit deregistered (bye)");
            registry.deregister_connection(&node_id, epoch).await;
            if let Some(conn) = orbit_conns.remove_epoch(&node_id, epoch).await {
                conn.fail_all().await;
            }
        ...
        OrbitFrame::Resp {
            ...
            if let Some(c) = conn {
                c.resolve(
                    req_id,
                    crate::server::orbit_conn::OrbitResp { ok, error, result },
                )
                .await;
            }
        }
        OrbitFrame::Event {
            ...
            if let Some(c) = conn {
                if let Err(error) = c.apply_event(&session_id, seq, payload).await {
                    tracing::warn!(session = %session_id, seq, error = %error, "orbit event rejected; leaving unacked");
                }
            }
        }
        OrbitFrame::Observed {
            ...
            if let Some(c) = conn {
                if let Err(error) = c.apply_observed(&session_id, seq, payload).await {
                    tracing::warn!(session = %session_id, seq, error = %error, "orbit observation rejected; leaving unacked");
                }
            }
        }
        OrbitFrame::Runtimes { runtimes: _ } => {
        ...
        OrbitFrame::TerminalOutput {
            ...
            if let Some(c) = conn {
                c.forward_terminal(
                    &terminal_id,
                    crate::server::orbit_conn::TerminalFrame::Output { data_b64 },
                );
            }
        }
        OrbitFrame::TerminalExited {
            ...
            if let Some(c) = conn {
                c.forward_terminal(
                    &terminal_id,
                    crate::server::orbit_conn::TerminalFrame::Exited { exit_code },
                );
...
// ── Iroh listener (remote orbits, ADR 0008 §1 / S7) ────────────────────
...
fn load_allowlist(home: &std::path::Path) -> Vec<iroh::PublicKey> {
    ...
    let Ok(raw) = std::fs::read_to_string(&path) else {
        return Vec::new(); // no file → empty allowlist → no remote orbits (fail closed)
    };
    let cfg: AxisConfig = match toml::from_str(&raw) {
        Ok(c) => c,
        Err(e) => {
            tracing::error!(error = %e, path = %path.display(), "axis.toml parse failed — remote orbits disabled");
            return Vec::new();
        }
    };
    cfg.allowed_envoys
        ...
        .filter_map(|s| match s.parse::<iroh::PublicKey>() {
            Ok(k) => Some(k),
            Err(e) => {
                tracing::warn!(id = %s, error = %e, "invalid node id in axis.toml allowlist, skipping");
                None
            }
        })
        .collect()
...
/// Bind the axis's iroh endpoint (public n0 relays) and return it + the node
/// id. The node id is needed at boot for `AppState` (GET /api/nodes/axis-identity)
/// and for the operator log line. The accept loop is [`run_iroh_accept_loop`].
///
/// The axis's iroh identity persists at `<home>/iroh.key`.
pub async fn create_iroh_endpoint(
    home: &std::path::Path,
) -> anyhow::Result<(iroh::Endpoint, iroh::EndpointId)> {
    let secret = stellarc_orbit::transport::load_or_create_secret(home)?;
    let endpoint = stellarc_orbit::transport::bind_endpoint(secret).await?;
    let node_id = endpoint.id();
    tracing::info!(iroh_node_id = %node_id, "axis iroh endpoint listening (remote orbits)");
    Ok((endpoint, node_id))
}
...
pub async fn run_iroh_accept_loop(
    ...
    while let Some(incoming) = endpoint.accept().await {
        ...
        if !allowlist.contains(&peer) {
            tracing::warn!(peer = %peer, "rejecting non-allowlisted iroh orbit");
            conn.close(1u32.into(), b"not allowlisted");
            continue;
        }
        tracing::info!(peer = %peer, "iroh orbit connected");
        let reg = registry.clone();
        let conns = orbit_conns.clone();
...
mod tests {
    ...
    fn hello(node_id: &str) -> stellarc_proto::frames::OrbitFrame {
        hello_version(node_id, PROTOCOL_VERSION)
    }

    fn hello_version(node_id: &str, protocol_version: u32) -> stellarc_proto::frames::OrbitFrame {
        stellarc_proto::frames::OrbitFrame::Hello {
    ...
    async fn unknown_node_heartbeat_requests_reregistration() {
        ...
        let heartbeat = stellarc_proto::frames::OrbitFrame::Heartbeat {
            node_id: "node".into(),
            slots_used: 0,
        };
        orbit_writer
        ...
        let mut lines = tokio::io::BufReader::new(orbit_reader).lines();
        let line = tokio::time::timeout(Duration::from_secs(1), lines.next_line())
            .await
            .expect("Axis must repair unknown heartbeat")
            .unwrap()
            .unwrap();
        assert!(matches!(
            serde_json::from_str::<stellarc_proto::frames::AxisFrame>(&line).unwrap(),
            stellarc_proto::frames::AxisFrame::ReRegister
        ));
        drop(orbit_writer);
        task.abort();
    ...
    async fn transport_rejects_unversioned_legacy_hello() {
        ...
        let task = tokio::spawn(handle_envoy_conn(
            ...
            writer,
            registry.clone(),
            crate::server::orbit_conn::OrbitConnections::new(),
            NodeTransport::Iroh,
            Some("key".into()),
        ));
        orbit
    ...
    async fn uds_rejects_unversioned_legacy_hello() {
        let registry = NodeRegistry::new();
        let (axis, mut orbit) = tokio::net::UnixStream::pair().unwrap();
