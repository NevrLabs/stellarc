    ...
    let job = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("sec-fetch-site", "same-origin")
                .header("content-type", "application/json")
                .body(Body::from(r#"{"nodeId":"axis","argv":["true"]}"#))
                .unwrap(),
        )
        .await
        .unwrap();
...
async fn cookie_sessions_and_operator_flag_fail_closed() {
    ...
    let live = state
        ...
        .unwrap();
    let live_cookie = format!("stellarc_session={}", live.token);
    let app = build_router(state.clone());
    ...
    let expired = app
        .oneshot(
            Request::builder()
                .uri("/api/auth/session")
                .header("cookie", format!("stellarc_session={}", expired.token))
                .header("sec-fetch-site", "same-origin")
                .body(Body::empty())
                .unwrap(),
        )
...
// ---- A2: POST /api/sessions (new managed Stellarc chat) ----

#[tokio::test]
async fn post_sessions_creates_managed_stellarc_session() {
    // POST /api/sessions with no body → creates a new Stellarc-managed session
    // OPTIMISTICALLY (no runtime spawned), returns 201 with a Session DTO
    // where source="stellarc", managed=true, and an empty hermesId (the real
...
async fn explicit_node_disconnect_fails_closed_without_local_runtime_fallback() {
    ...
    state
        ...
        .await;
    // Deliberately omit an OrbitConnection: the selected node disconnected
...
#[tokio::test]
async fn post_message_to_managed_stellarc_session_is_202() {
    // A managed stellarc session should accept a prompt and return 202
...
/// id (consumes the token, lands in axis.toml) → second registration with
/// a different id is rejected. mint requires the axis iroh identity.
#[tokio::test]
async fn enroll_flow_mint_script_register() {
    let (mut state, dir) = test_state();
    state.axis_iroh_id = Some(Arc::new(
    ...
    assert!(
        !script.contains("{{AXIS_URL}}"),
        "placeholders must be replaced"
    );
    assert!(script.contains("83141ef93390a387aec148672f7ae44a9ee4c02a0f23f82c0bb80fcc2e499320"));
    assert!(
        script.contains("Environment=\"STELLARC_HOME=$STELLARC_HOME\""),
        "the node-local state root must survive into the systemd service"
    );
    assert!(
        script.contains("install -d -m 0700 \"$STELLARC_HOME\" \"$BIN_DIR\""),
        "the node-local state root must be private"
    );
    assert!(
        script.contains("UMask=0077"),
        "the Orbit service must create private state by default"
    );

    // 3. Register a node id — lands in axis.toml.
    let orbit_id = "93141ef93390a387aec148672f7ae44a9ee4c02a0f23f82c0bb80fcc2e499321";
    let res = app
        .clone()
        .oneshot(
            Request::builder()
                ...
                .header("content-type", "application/json")
                .body(Body::from(format!(
                    "{{\"irohNodeId\":\"{orbit_id}\",\"nodeId\":\"test-node\"}}"
                )))
                .unwrap(),
        )
        .await
...
/// mint_enroll fails honestly (503) when the axis has no iroh identity —
/// a remote orbit couldn't connect anyway.
#[tokio::test]
    ...
    let response = build_router(state)
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/enroll")
                .header("cookie", format!("stellarc_session={}", session.token))
                .header("host", "127.0.0.1:8787")
                .header("sec-fetch-site", "same-origin")
                .body(Body::empty())
                .unwrap(),
...
/// DELETE /api/nodes/:id revokes the node's iroh key from axis.toml and
