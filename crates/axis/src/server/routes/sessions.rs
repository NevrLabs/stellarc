...
pub(crate) struct SessionsQuery {
    ...
    pinned: Option<bool>,
    /// Filter by managed status: `true` → Stellarc-driven sessions (your active
...
pub(crate) async fn list_sessions(
    ...
    let mut rows: Vec<SessionDto> = views
        ...
        // tagged source=stellarc but were never driven by Stellarc (agent unset and
        ...
        .filter(|dto| {
            let is_managed = dto.source == "acp" || dto.source == "stellarc";
    ...
    for r in rows.iter_mut() {
        let managed = r.source == "acp" || r.source == "stellarc";
...
pub(crate) async fn get_messages(
    ...
    // MessageView. Managed (stellarc/acp) sessions still use the in-memory
...
/// POST /api/sessions — create a new Stellarc-managed chat session **optimistically**.
///
...
pub(crate) async fn create_session(
    ...
    let spec = crate::server::bridge_mgr::RuntimeSpec {
        ...
        workspace_version: stellarc_proto::runtime::MANAGED_WORKSPACE_VERSION,
        cwd: None,
        mcp_servers: vec![],
        env: vec![],
    };
...
pub(crate) async fn patch_session(
    ...
    let capability_event = {
        ...
        if wants_rebind && !(row.source == "stellarc" || row.source == "acp") {
            return (
                StatusCode::CONFLICT,
                Json(json!({
                    "error": "observed",
...
/// stellarc-owned session first (cross-channel continuation, ADR §6.6).
///
...
pub(crate) async fn post_message(
    ...
    let (managed, hermes_id, agent, node, organization_id) = {
        ...
        let managed = session.source == "stellarc" || session.source == "acp";
        (
            managed,
    ...
    if !managed {
        return (
            ...
            Json(json!({
                "error": "observed",
                "message": "This session is observed (read-only). Fork it into an Stellarc-managed session to continue.",
            })),
        )
            .into_response();
    }
    ...
    let orbit_conns = state.orbit_conns.clone();
    ...
    let org_slug = std::env::var("STELLARC_DEFAULT_ORG").unwrap_or_else(|_| "default".to_string());
    ...
    tokio::spawn(async move {
        ...
        //
        // ADR 0008 S6 cutover: route to a connected orbit (RemoteRuntime) when
        // the session's node has an active UDS connection. This replaces the
        // in-process bridge for production — the local node is now
        // stellarc-orbit@1 over UDS, not an in-process pseudo-orbit. Tests that
        // build AppState with no connected orbits fall back to the in-process
        ...
        // orbit (default for the single-operator case). Sessions with an
        // explicit node route to that specific orbit.
        let route_node = if node_id.is_empty() {
            orbit_conns.first_node().await.unwrap_or_default()
        } else {
            node_id
        };
        let conn = orbit_conns.get(&route_node).await;
        let runtime_result = if let Some(conn) = conn {
            // Route to the connected orbit via RemoteRuntime.
            let rt = crate::server::orbit_conn::RemoteRuntime::arc_with_spec(
                conn,
                session_id.clone(),
                spec.clone(),
            );
            emit_log(
                "info",
                "bridge",
                &format!("Routing to orbit {}…", route_node),
            );
            ...
            // losing that Orbit must fail closed rather than silently running
            // the session on Axis or another machine.
            bridge
