...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/api/nodes/axis-identity", get(axis_identity))
...
/// 0021 cockpit picker). Always includes the Axis host first (the default
...
/// GET /api/nodes/:id/agents — the agents a specific node's orbit discovered.
...
/// automatic).
///
/// - `local` (the in-process control-plane host, no orbit connection): re-run
///   discovery in-process.
/// - any node with a connected orbit: send a `Probe` frame over its existing
...
pub(crate) async fn refresh_node_agents(
    ...
    // The in-process host has no orbit connection to itself.
    if id == "local" {
    ...
    // Remote node: ask its orbit to re-probe over the live connection.
    let Some(conn) = state.orbit_conns.get(&id).await else {
        // Distinguish "unknown node" from "known but its orbit isn't connected".
        ...
        return (
            status,
            Json(json!({ "error": format!("node '{id}' has no connected orbit to probe") })),
        )
            .into_response();
    };
...
/// Send a `Probe` frame to an orbit and parse its reported agents. Bounded by a
/// timeout so a wedged orbit can't hang the request.
async fn probe_agents(
    conn: &crate::server::orbit_conn::OrbitConnection,
) -> Result<Vec<agents::AgentInfo>, String> {
    let rx = conn
        .send_request(stellarc_proto::frames::AxisFrame::Probe { req_id: 0 })
        .await
    ...
    let resp = tokio::time::timeout(std::time::Duration::from_secs(5), rx)
        .await
        .map_err(|_| "orbit did not respond within 5s".to_string())?
        .map_err(|_| "orbit disconnected before responding".to_string())?;
    if !resp.ok {
        return Err(resp
            .error
            .unwrap_or_else(|| "orbit reported failure".into()));
    }
...
/// GET /api/nodes/axis-identity — returns the axis's iroh node id (public key,
...
pub(crate) async fn axis_identity(State(state): State<AppState>) -> impl IntoResponse {
    let id = state.axis_iroh_id.as_ref().map(|s| s.as_str().to_string());
...
/// drops its orbit connection, and — for iroh nodes — revokes its key from
/// the axis.toml allowlist so it cannot silently reconnect.
pub(crate) async fn remove_node(State(state): State<AppState>, Path(id): Path<String>) -> Response {
    ...
    if let Some(conn) = state.orbit_conns.remove(&id).await {
