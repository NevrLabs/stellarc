...
async fn dispatch(
    ...
    let Some(conn) = state.orbit_conns.get(&body.node_id).await else {
        return (
    ...
    let frame = AxisFrame::DispatchJob {
        req_id: 0,
        job_id: job_id.clone(),
        attempt_epoch,
    ...
    if response.as_ref().is_some_and(|response| response.ok) {
        ...
        let reason = response
            ...
            .unwrap_or_else(|| "orbit disconnected before dispatch acknowledgement".into());
...
async fn cancel(
    ...
    let Some(conn) = state.orbit_conns.get(&job.node_id).await else {
        return StatusCode::CONFLICT.into_response();
    };

    let Ok(Some(rx)) = conn
        .send_request(AxisFrame::CancelJob {
            req_id: 0,
            job_id: id,
