//! Orbit connection manager + RemoteRuntime — the Axis-side counterparts to
//! the proto wire protocol (ADR 0008 §1, milestone S3).
//!
//! Each connected orbit gets an [`OrbitConnection`] holding the write half of
//! its UDS stream plus a pending-request table and per-session event channels.
//! Axis sends [`AxisFrame`]s to drive session ops; the orbit replies with
//! [`OrbitFrame::Resp`] and streams [`OrbitFrame::Event`] frames back. The UDS
...
//! `OrbitConnection`, so Axis's existing `post_message` drain loop can drive a
//! session on a remote orbit identically to an in-process runtime.
...
use stellarc_orbit::bridge::{AgentCommand, AgentEvent, AgentRuntime};
use stellarc_proto::frames::AxisFrame;
use tokio::io::AsyncWriteExt;
use tokio::sync::{broadcast, Mutex, RwLock};

/// A boxed, thread-safe async writer — transport-agnostic (UDS OwnedWriteHalf
/// or iroh QUIC SendStream). This lets `OrbitConnection` work identically over
/// either transport (ADR 0008 §1).
pub type BoxedWriter = Box<dyn tokio::io::AsyncWrite + Send + Unpin>;

/// A pending request awaiting an orbit `Resp` frame.
type PendingSlot = tokio::sync::oneshot::Sender<OrbitResp>;

/// The structured result extracted from an `OrbitFrame::Resp`.
#[derive(Debug)]
pub struct OrbitResp {
    pub ok: bool,
    pub error: Option<String>,
    pub result: Option<serde_json::Value>,
}

/// One orbit's connection state: the write half + pending requests + per-session
/// event channels.
pub struct OrbitConnection {
    pub(crate) epoch: u64,
    shutdown: tokio::sync::watch::Sender<bool>,
    /// Buffered writer to the transport stream (UDS or iroh QUIC). Guarded so
    /// Axis can send from any task. Transport-agnostic via [`BoxedWriter`].
    writer: Mutex<tokio::io::BufWriter<BoxedWriter>>,
    /// Pending requests keyed by reqId, awaiting `OrbitFrame::Resp`.
    pending: Mutex<HashMap<u64, PendingSlot>>,
    /// Next Axis-assigned reqId.
    ...
    /// Per-terminal output channels (ADR 0021 cockpit). Keyed by Axis-issued
...
/// A terminal output/exit event forwarded from an orbit to the operator WS.
#[derive(Debug, Clone)]
...
impl OrbitConnection {
    ...
    /// Send a AxisFrame to this orbit. For request frames (those with a
    ...
    pub async fn send_request(
        &self,
        frame: AxisFrame,
    ) -> Result<Option<tokio::sync::oneshot::Receiver<OrbitResp>>> {
        match &frame {
            ...
            | AxisFrame::TerminalOpen { .. } => {
                ...
                let json = serde_json::to_string(&frame_with_id)
                    .map_err(|e| anyhow::anyhow!("serializing AxisFrame: {e}"))?;
                {
    ...
    /// Resolve a pending request with the orbit's response (called from the
    /// UDS read loop when an `OrbitFrame::Resp` arrives).
    pub async fn resolve(&self, req_id: u64, resp: OrbitResp) {
    ...
    /// from the UDS read loop when an `OrbitFrame::Event` arrives). Synchronous
    ...
    pub async fn apply_event(&self, session_id: &str, seq: u64, event: AgentEvent) -> Result<bool> {
        ...
        self.send_request(AxisFrame::Ack {
    ...
    pub async fn apply_observed(
        ...
        seq: u64,
        payload: stellarc_proto::frames::ObservedEvent,
    ) -> Result<bool> {
    ...
    pub async fn apply_job_output(
        ...
        attempt_epoch: u64,
        seq: u64,
        stream: stellarc_proto::frames::JobStream,
        data: String,
    ) -> Result<()> {
    ...
    pub fn reconcile_jobs(
        &self,
        node_id: &str,
        attempts: &[stellarc_proto::frames::JobAttemptStatus],
    ) -> Result<()> {
        self.jobs
            .as_ref()
    ...
    pub fn watermark(&self, session_id: &str) -> Result<Option<u64>> {
        self.log
            .as_ref()
            .map_or(Ok(None), |log| log.orbit_watermark(session_id))
    }
    ...
    /// Forward a terminal frame from the orbit read loop to any operator WS
    ...
    /// Clean up: fail all pending requests (orbit disconnected).
...
/// Inject a Axis-assigned reqId into a request frame (overwriting whatever was
/// there). Callers build frames with a dummy reqId (0) and Axis allocates the
/// real one.
fn inject_req_id(frame: AxisFrame, req_id: u64) -> AxisFrame {
    match frame {
        AxisFrame::EnsureRuntime {
            session_id,
            spec,
        ...
        AxisFrame::Cancel { session_id, .. } => AxisFrame::Cancel { req_id, session_id },
        AxisFrame::Stop { session_id, .. } => AxisFrame::Stop { req_id, session_id },
        AxisFrame::RespondPermission {
            session_id,
            request_id,
            option_id,
        ...
        AxisFrame::Drain { to_node, .. } => AxisFrame::Drain { req_id, to_node },
        AxisFrame::Probe { .. } => AxisFrame::Probe { req_id },
        AxisFrame::DispatchJob {
            job_id,
            attempt_epoch,
            package_id,
...
/// Thread-safe map of `node_id → OrbitConnection`. The UDS read loop stores
...
impl OrbitConnections {
    ...
    pub async fn insert(&self, node_id: &str, writer: BoxedWriter) -> Arc<OrbitConnection> {
        let conn = OrbitConnection::new(writer, self.log.clone(), self.jobs.clone());
        self.inner
            .write()
            .await
    ...
    pub async fn insert_epoch(
        ...
        epoch: u64,
        shutdown: tokio::sync::watch::Sender<bool>,
    ) -> Result<(Arc<OrbitConnection>, Option<Arc<OrbitConnection>>), Arc<OrbitConnection>> {
        let conn = OrbitConnection::new_with_epoch(
            writer,
            self.log.clone(),
            self.jobs.clone(),
            epoch,
    ...
    pub async fn remove(&self, node_id: &str) -> Option<Arc<OrbitConnection>> {
        self.inner.write().await.remove(node_id)
    }

    pub async fn remove_epoch(&self, node_id: &str, epoch: u64) -> Option<Arc<OrbitConnection>> {
        let mut inner = self.inner.write().await;
    ...
    pub async fn get(&self, node_id: &str) -> Option<Arc<OrbitConnection>> {
        self.inner.read().await.get(node_id).cloned()
    }

    /// Whether a node has a registered orbit connection (i.e. is remote).
    pub async fn is_remote_node(&self, node_id: &str) -> bool {
        self.inner.read().await.contains_key(node_id)
    }

    /// The first connected orbit's node id (for default routing when a session
    /// has no explicit node). Returns None when no orbits are connected.
...
/// An [`AgentRuntime`] backed by a remote orbit connection. Axis sends
/// [`AxisFrame`]s over the UDS; the orbit drives the real agent child and
/// streams events back as [`OrbitFrame::Event`] frames. The UDS read loop
/// forwards those events into the per-session broadcast channel, which
/// `events()` drains — so Axis's existing `post_message` drain loop works
/// identically for remote sessions.
pub struct RemoteRuntime {
    conn: Arc<OrbitConnection>,
    session_id: String,
    hermes_id: tokio::sync::Mutex<Option<String>>,
    /// The spawn configuration sent to the orbit in EnsureRuntime so it
    /// knows which agent/cwd/mcp to use. Defaults to empty (the orbit's
    /// factory picks its own cwd + default agent).
    spec: stellarc_proto::RuntimeSpec,
}

impl RemoteRuntime {
    /// Create a new remote runtime for a session on the given orbit connection.
    pub fn new(conn: Arc<OrbitConnection>, session_id: String) -> Self {
        Self {
            conn,
            session_id,
            hermes_id: tokio::sync::Mutex::new(None),
            spec: stellarc_proto::RuntimeSpec::default(),
        }
    }

    /// Create a remote runtime with an explicit spawn spec. The spec flows
    /// into the EnsureRuntime frame so the orbit's factory knows which agent
    ...
    /// AgentRuntime trait).
    pub fn new_arc(conn: Arc<OrbitConnection>, session_id: String) -> Arc<dyn AgentRuntime> {
...
impl AgentRuntime for RemoteRuntime {
    async fn start(&self, session_id: Option<&str>) -> Result<()> {
        // EnsureRuntime: tell the orbit to spawn (or resume) a runtime for
        // this session. The orbit replies with the captured Hermes session id.
        let resume_id = session_id.filter(|s| !s.is_empty()).map(String::from);
        let frame = AxisFrame::EnsureRuntime {
            req_id: 0, // Axis assigns the real id
        ...
        let resp = rx
            .await
            .map_err(|_| anyhow::anyhow!("orbit disconnected before EnsureRuntime resp"))?;
        if !resp.ok {
            anyhow::bail!(
    ...
    async fn send(&self, cmd: AgentCommand) -> Result<()> {
        let frame = match cmd {
            ...
            AgentCommand::Steer { text } => AxisFrame::Steer {
                req_id: 0,
                session_id: self.session_id.clone(),
                text,
            },
            AgentCommand::Cancel => AxisFrame::Cancel {
                req_id: 0,
                session_id: self.session_id.clone(),
            },
            AgentCommand::Stop => AxisFrame::Stop {
                req_id: 0,
                session_id: self.session_id.clone(),
            },
            // Slash / SwitchModel are Hermes-CLI-specific; for the remote path,
            // map them to a prompt (the orbit's real runtime handles them).
            AgentCommand::Slash { command } => AxisFrame::Prompt {
                req_id: 0,
                session_id: self.session_id.clone(),
                text: format!("/{command}"),
                model: None,
            },
            AgentCommand::SwitchModel { model } => AxisFrame::Prompt {
                req_id: 0,
                session_id: self.session_id.clone(),
        ...
        let resp = rx
            .await
            .map_err(|_| anyhow::anyhow!("orbit disconnected before command resp"))?;
        if !resp.ok {
            anyhow::bail!(
                "command failed: {}",
                resp.error.unwrap_or_else(|| "unknown error".into())
    ...
    async fn stop(&self) -> Result<()> {
        // Send Stop and best-effort await the Resp (the orbit closes the child).
        let frame = AxisFrame::Stop {
            req_id: 0,
            session_id: self.session_id.clone(),
        };
        if let Some(rx) = self.conn.send_request(frame).await? {
            let _ = rx.await; // best-effort: orbit may disconnect immediately
        }
        Ok(())
    }
    ...
    async fn resumable(&self) -> bool {
        // The capability is parsed by the orbit from the adapter's initialize
        // response. Axis can query it via a probe or runtimes-table frame in a
...
mod tests {
    ...
    /// Create a test OrbitConnection backed by a real UDS socketpair (the
    ...
    fn test_conn() -> Arc<OrbitConnection> {
        OrbitConnection::new(test_writer(), None, None)
    }

    #[tokio::test]
    async fn orbit_connections_insert_get_remove() {
    ...
    async fn durable_watermark_drops_duplicates_across_axis_restart() {
        ...
        assert_eq!(log.orbit_watermark("s1").unwrap(), Some(1));
        ...
        assert!(events.try_recv().is_err());
        assert_eq!(reopened.orbit_watermark("s1").unwrap(), Some(2));
    }
    ...
    async fn observed_state_db_rows_survive_axis_restart_exactly_once() {
        ...
        let spool = EventSpool::open(&dir.path().join("orbit")).unwrap();
        let axis_db = dir.path().join("axis.db");
    ...
    async fn sequence_gap_is_not_forwarded_or_acknowledged() {
        ...
        assert!(events.try_recv().is_err());
        assert_eq!(log.orbit_watermark("s1").unwrap(), None);
        use tokio::io::AsyncReadExt as _;
        let mut byte = [0];
        assert!(
