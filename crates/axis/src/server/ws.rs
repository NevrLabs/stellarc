...
//!
//! Browser clients connect with their Axis cookie and `?organization=…`; legacy
...
/// Accepts the installation token (legacy operator) or a valid Axis session
...
mod tests {
    ...
    fn websocket_authorization_requires_cookie_membership_scope() {
        ...
        let headers = axum::http::HeaderMap::from_iter([(
            ...
            format!("stellarc_session={}", session.token)
                .parse()
                .unwrap(),
        )]);
