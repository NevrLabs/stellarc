...
pub use stellarc_orbit::discovery as agents;
...
pub struct AppState {
    ...
    /// Fleet node registry — tracks connected orbits (UDS) + the local node.
    pub nodes: crate::node::NodeRegistry,
    /// Remote orbit connections (UDS or iroh write halves for RemoteRuntime).
    pub orbit_conns: crate::server::orbit_conn::OrbitConnections,
    /// Axis-local operator terminal (PTY) manager — the "axis" node target in
    ...
    pub axis_iroh_id: Option<Arc<String>>,
    ...
    /// Orbit enrollment tokens (one-line node setup). Ephemeral by design.
    pub enroll: crate::enroll::EnrollStore,
    /// The stellarc home dir (`~/.stellarc`) — axis.toml allowlist + bin/ live here.
...
/// resolved from STELLARC_UI_DIST (default "ui/dist" relative to the CWD).
...
fn static_ui_service() -> tower_http::services::ServeDir<tower_http::services::ServeFile> {
    ...
    let dist = std::env::var("STELLARC_UI_DIST").unwrap_or_else(|_| "ui/dist".to_string());
...
/// cross-origin requests + preflights. Mirror the exact Axis-origin policy and
