...
//!
//! - target `node == "axis"` (default): a shell on the Axis host, run by the
//!   Axis-local [`AxisTerminals`] manager (Axis has no OrbitConnection to
//!   itself). Reuses the node-agnostic `stellarc_orbit::pty::PtyManager`.
//! - target `node == <orbit node id>`: Axis sends `TerminalOpen` to that
//!   node's [`OrbitConnection`], subscribes to its terminal channel, and relays
//!   frames both ways.
//!
...
use super::orbit_conn::TerminalFrame;
use super::AppState;
use stellarc_orbit::pty::{b64_encode, ChannelSink, PtyManager, TerminalMsg};

/// Axis-local terminals: a `PtyManager` for shells on the Axis host, plus the
/// receiver drained per-attachment. Because `PtyManager`'s sink is fixed at
/// construction, `AxisTerminals` builds one manager whose output is fanned out
/// to per-terminal subscribers via broadcast channels (mirrors the orbit
/// connection's terminal_channels).
pub struct AxisTerminals {
    mgr: Arc<PtyManager>,
...
pub struct TerminalQuery {
    /// Target node id; `"axis"` (default) runs on the Axis host.
    #[serde(default)]
    node: Option<String>,
...
async fn relay(
    ...
    let mut output_rx = if node == "axis" {
        state.axis_pty.subscribe(&terminal_id)
    } else {
        match state.orbit_conns.get(&node).await {
    ...
    let persistent = if node == "axis" {
        ...
        match state.orbit_conns.get(&node).await {
            Some(conn) => {
                match conn
                    .send_request(stellarc_proto::frames::AxisFrame::TerminalOpen {
                        req_id: 0,
                    ...
                    .await
                {
                    Ok(_) => true, // orbit persistence is its own concern
                    Err(_) => {
                        let _ = socket
                            .send(Message::Text(
                                "{\"kind\":\"exited\",\"error\":\"open failed\"}".into(),
...
async fn send_input(state: &AppState, node: &str, terminal_id: &str, data_b64: &str) {
    if node == "axis" {
        let _ = state.axis_pty.manager().input(terminal_id, data_b64).await;
    } else if let Some(conn) = state.orbit_conns.get(node).await {
        let _ = conn
            .send_request(stellarc_proto::frames::AxisFrame::TerminalInput {
...
async fn send_resize(state: &AppState, node: &str, terminal_id: &str, cols: u16, rows: u16) {
    if node == "axis" {
        ...
        let _ = conn
            .send_request(stellarc_proto::frames::AxisFrame::TerminalResize {
                terminal_id: terminal_id.to_string(),
                cols,
                rows,
            })
...
async fn detach_terminal(state: &AppState, node: &str, terminal_id: &str) {
    if node == "axis" {
        let _ = state.axis_pty.manager().detach(terminal_id).await;
    }
    // For orbit nodes: Axis can't detach on the remote — the orbit's PtyManager
    // handles it when the TerminalOpen resp/reader EOFs. Just drop the channel.
    state.axis_pty.drop_terminal(terminal_id);
}

/// Close permanently: kill the tmux session / bare shell.
async fn close_terminal(state: &AppState, node: &str, terminal_id: &str) {
    if node == "axis" {
        let _ = state.axis_pty.manager().close(terminal_id).await;
        state.axis_pty.drop_terminal(terminal_id);
    } else if let Some(conn) = state.orbit_conns.get(node).await {
        let _ = conn
            .send_request(stellarc_proto::frames::AxisFrame::TerminalClose {
                terminal_id: terminal_id.to_string(),
            })
            .await;
