...
pub const STELLARC_API_VERSION: &str = "0.1";
...
impl PackageManifest {
    ...
    pub fn validate_compatibility(&self, stellarc_api: &str, platform: &str) -> Result<()> {
        let requested = self.compatibility.stellarc_api.trim();
        anyhow::ensure!(
            requested == "*"
                || requested == stellarc_api
                || requested
                    .strip_prefix('^')
                    .is_some_and(|v| v == stellarc_api),
            "package requires Stellarc API {requested}, host is {stellarc_api}"
        );
        anyhow::ensure!(
...
pub fn validate_install(
    ...
    manifest.validate_compatibility(STELLARC_API_VERSION, std::env::consts::OS)?;
...
"#;
    ...
    /// validate_compatibility must reject mismatched stellarc_api and non-matching platforms.
    #[test]
