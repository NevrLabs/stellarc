...
impl JobService {
    pub fn open(log: Arc<Log>) -> Result<Self> {
        ...
        // Axis restart destroys certainty until the owning Orbit reports inventory.
        ...
        for (job_id, epoch) in candidates {
                ...
                .find(|attempt| attempt.job_id == job_id && attempt.attempt_epoch == epoch)
            {
                Some(attempt) => {
                    let (status, reason) = match attempt.state {
                        ...
                        JobAttemptState::Completed => (
                            "StepIndeterminate",
                            Some("orbit_completed_without_durable_terminal_sequence".into()),
                        ),
                ...
                None => self.record_reconciliation(
                    ...
                    "StepIndeterminate",
                    None,
                    Some("orbit_did_not_report_attempt".into()),
                )?,
            }
        }
        Ok(())
...
fn dispatch_frame(job: &JobRecord) -> AxisFrame {
...
mod tests {
    ...
    fn restart_reconciliation_duplicate_result_and_output_gap_refusal() {
        ...
        assert_eq!(
            ...
            Some("orbit_completed_without_durable_terminal_sequence")
        );
