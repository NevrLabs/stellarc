...
pub struct SessionRow {
    ...
    /// Agent (Hermes profile) bound to this session, if assigned. Stellarc
    ...
    /// Axis-signed capability envelope. None preserves legacy full authority.
...
impl SessionView {
        ...
        match event {
            ...
            Event::MessageAppended {
                ...
                if let Some(row) = self.sessions.get_mut(session_id) {
                    ...
                    // (source acp/stellarc) never emit SessionUpdated{message_count},
                    ...
                    let managed = row.source == "acp" || row.source == "stellarc";
                    if managed {
    ...
    /// Whether a session is managed (stellarc/acp source) — its messages are
    /// live-streamed from the bridge, not read from state.db.
    pub fn is_managed(&self, session_id: &str) -> bool {
        self.sessions
            .get(session_id)
            .map(|r| r.source == "acp" || r.source == "stellarc")
            .unwrap_or(false)
    }
}
