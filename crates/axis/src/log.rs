...
impl Log {
    ...
    pub fn append_orbit_event(&self, identity: &str, seq: u64, event: &Event) -> Result<bool> {
        ...
        let current: Option<i64> = tx
            .query_row(
                "SELECT seq FROM orbit_watermarks WHERE session_id=?1",
                [identity],
                |row| row.get(0),
            )
            .optional()?;
    ...
    pub fn delete_job_history(&self, seqs: &[u64], identities: &[String]) -> Result<()> {
        ...
        for identity in identities {
            tx.execute(
                "DELETE FROM orbit_watermarks WHERE session_id=?1",
                [identity],
            )?;
        }
        tx.commit()?;
    ...
    pub fn accept_orbit_seq(&self, session_id: &str, seq: u64) -> Result<bool> {
        ...
        if seq != expected {
            anyhow::bail!(
                "orbit event sequence gap for {session_id}: expected {expected}, got {seq}"
            );
        }
        tx.execute(
            "INSERT INTO orbit_watermarks(session_id,seq) VALUES(?1,?2)
    ...
    pub fn accept_observed(
        ...
        anyhow::ensure!(
            seq == expected,
            "orbit observation sequence gap for {transport_session_id}: expected {expected}, got {seq}"
        );
        tx.execute(
    ...
    /// Keep Stellarc-native events only (setup declarations, cards, registry,
    /// stellarc sessions + their messages). The state.db mirror is rebuilt on
    ...
    pub fn retain_native(&self) -> Result<()> {
        ...
        tx.execute_batch(
            "CREATE TEMP TABLE IF NOT EXISTS _native AS
             SELECT session_id FROM sessions WHERE source = 'stellarc'
        ...
        tx.execute("DELETE FROM sessions WHERE source != 'stellarc'", [])?;
...
mod tests {
    ...
    #[test]
    fn orbit_ack_durability_uses_full_synchronous_mode() {
        let dir = tempfile::tempdir().unwrap();
        let log = Log::open(&dir.path().join("stellarc.db")).unwrap();
        let mode: i64 = log
            .conn
            .lock()
            .unwrap()
