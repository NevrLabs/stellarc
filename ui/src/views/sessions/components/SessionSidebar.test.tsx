...
describe("SessionSidebar", () => {
  ...
  it("writes a dockview session drag payload", () => {
    ...
    fireEvent.dragStart(screen.getAllByText("Focused session")[0].closest(".srow") as HTMLElement, {
      dataTransfer: {
        effectAllowed: "none",
  ...
  it("surfaces a failed project-session open", async () => {
    ...
    fireEvent.click(screen.getAllByText("Focused session")[0]);
  ...
  it("collapses and expands sections on header click", () => {
    ...
    expect(container.querySelectorAll("[data-session-id='s-1']")).toHaveLength(2);

    fireEvent.click(recentHeader);
    expect(container.querySelectorAll("[data-session-id='s-1']")).toHaveLength(1);
  });
});
