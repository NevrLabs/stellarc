...
export function NavigationPlaygrounds() {
  const [vertical, setVertical] = useState(false);
  const [page, setPage] = useState(2);
  return <><ComponentPlayground title="Tabs" importLine={'import { Tabs } from "@/components/ui/tabs"'} controls={<ControlRow label="Vertical"><Switch checked={vertical} onCheckedChange={setVertical} /></ControlRow>}><Tabs defaultValue="output" orientation={vertical ? "vertical" : "horizontal"} className="w-80"><TabsList><TabsTrigger value="output">Output</TabsTrigger><TabsTrigger value="events">Events</TabsTrigger><TabsTrigger value="files" disabled>Files</TabsTrigger></TabsList><TabsContent value="output">Run output stream.</TabsContent><TabsContent value="events">Event log.</TabsContent></Tabs></ComponentPlayground><ComponentPlayground title="Breadcrumb & Pagination" importLine={'import { Breadcrumb } from "@/components/ui/breadcrumb"'} controls={<ControlRow label="Current page"><Input aria-label="Current page" type="number" min={1} max={5} value={page} onChange={(e) => setPage(Math.min(5, Math.max(1, Number(e.target.value))))} className="h-7 w-20" /></ControlRow>}><div className="space-y-4"><Breadcrumb><BreadcrumbList><BreadcrumbItem><BreadcrumbLink href="#">Fleet</BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbLink href="#">Nodes</BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbPage>fxcompute-01</BreadcrumbPage></BreadcrumbItem></BreadcrumbList></Breadcrumb><nav aria-label="Pagination" className="flex items-center gap-2"><Button variant="outline" size="sm" disabled={page === 1} onClick={() => setPage(page - 1)}>Previous</Button><span className="text-sm">Page {page} of 5</span><Button variant="outline" size="sm" disabled={page === 5} onClick={() => setPage(page + 1)}>Next</Button></nav></div></ComponentPlayground></>;
}
