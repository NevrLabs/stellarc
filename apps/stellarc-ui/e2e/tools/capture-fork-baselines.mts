...
// These are the "pixel-frozen" contract for STL-14+. 21 screens × 4 viewports = 84 PNGs.
...
const PROJECTS = [
  ...
  { name: "mobile",       viewport: { width: 390,  height: 844 }, hasTouch: true,  isMobile: true, ua: devices["iPhone 14"].userAgent, dsf: 3 },
  { name: "mobile-small", viewport: { width: 360,  height: 640 }, hasTouch: true,  isMobile: true, dsf: 2 },
];
...
const manifest: Record<string, Record<string, { route: string; url: string; bytes: number; note?: string }>> = {};

// Resolve dynamic ids once, from the desktop authed context.
const probe = await browser.newContext({ storageState: "tests/e2e/.auth/user.json", viewport: { width: 1440, height: 900 } });
const pp = await probe.newPage();
const settled = async (url: string) => { await pp.goto(url, { waitUntil: "networkidle" }); await pp.waitForFunction(() => document.body.innerText.trim().length > 40 && !document.querySelector('[class*="skeleton"],.animate-pulse'), null, { timeout: 25000 }).catch(() => {}); await pp.waitForTimeout(400); };
const firstHref = (sel: string, marker: string) => pp.evaluate(([sel, marker]) => { const a = ([...document.querySelectorAll(sel)] as HTMLAnchorElement[]).find(x => x.href.includes(marker)); return a ? a.href.split(marker)[1].split(/[/?#]/)[0] : ""; }, [sel, marker] as const);
await settled(`${BASE}/dashboard/organization/${ORG}/board/${BOARD}/board`);
const firstTask = await firstHref('a[href*="/task/"]', "/task/") || await firstHref('a[href*="taskId="]', "taskId=");
// Repo cards and project rows navigate via onClick (no <a>); ids come from the DB, injected by the runner.
const repoSlug = process.env.REPO_ID ?? ""; const prNum = process.env.PR_NUMBER ?? ""; const projectSlug = process.env.PROJECT_SLUG ?? "";
await probe.close();
console.log("resolved:", { firstTask, repoSlug, prNum, projectSlug });

const DYN = new Set(["ticket-detail","repo-issues","repo-pulls","repo-pull-detail","project-detail"]);
...
for (const P of PROJECTS) {
  ...
  for (const [name, routeT] of Object.entries(SCREENS)) {
    ...
    const route = routeT.replace("__FIRST__", firstTask).replace("__REPO__", repoSlug).replace("__PR__", prNum).replace("__PROJECT__", projectSlug);
    const authed = name !== "sign-in";
    const ctx = await browser.newContext({
      ...(authed ? { storageState: "tests/e2e/.auth/user.json" } : {}),
      viewport: P.viewport, hasTouch: P.hasTouch, isMobile: P.isMobile,
      deviceScaleFactor: P.dsf ?? 1, userAgent: P.ua, colorScheme: "dark", locale: "en-US", timezoneId: "Asia/Jakarta",
    });
    const page = await ctx.newPage();
    // freeze time-relative text so baselines are stable
    await page.addInitScript(() => { const fixed = new Date("2026-09-09T00:00:00Z").valueOf(); const OD = Date; // @ts-ignore
      globalThis.Date = class extends OD { constructor(...a: any[]) { super(...(a.length ? a : [fixed])); } static now() { return fixed; } } as any; });
    let note: string | undefined;
    try {
      const m = route.match(/#viewMode=(\w+)$/); const nav = route.replace(/#viewMode=\w+$/, "");
      if (m) await page.addInitScript((mode) => {
        try { const k = "user-preferences"; const cur = JSON.parse(localStorage.getItem(k) ?? '{"state":{},"version":0}'); cur.state = { ...(cur.state ?? {}), viewMode: mode }; localStorage.setItem(k, JSON.stringify(cur)); } catch {}
      }, m[1]);
      ...
      await page.waitForFunction(() => {
        ...
        const skel = document.querySelectorAll('[class*="skeleton"],[data-slot="skeleton"],.animate-pulse').length;
      ...
      const note = body === "Not Found" ? "ROUTE 404" : body.length < 40 ? `SUSPICIOUS textLen=${body.length}` : undefined;
      manifest[P.name][name] = { route, url: page.url(), bytes, note };
    } catch (e) { note = String(e).slice(0, 160); manifest[P.name][name] = { route, url: "", bytes: 0, note }; }
    await ctx.close();
    process.stdout.write(`${P.name}/${name} ${note ? "FAIL " + note : "ok"}\n`);
  }
}
if (only) { try { const prev = JSON.parse((await import("node:fs")).readFileSync(`${OUT}/manifest.json`, "utf8")); for (const P of PROJECTS) manifest[P.name] = { ...prev.screens[P.name], ...manifest[P.name] }; } catch {} }
writeFileSync(`${OUT}/manifest.json`, JSON.stringify({ source: BASE, sha: "2504e64512b84b9b739d4d4fb0d4ceaefdb14783", capturedAt: new Date().toISOString(), resolved: { firstTask, repoSlug, prNum, projectSlug }, screens: manifest }, null, 2));
await browser.close();
